...
│           │            │                                         │
│           ▼            ▼                                         │
│   [Staging 환경]  [Production 환경]                             │
│    QA 확인         실 사용자                                    │
│           │                                                      │
│           ▼                                                      │
│   ┌──────────────────────────────────────┐                      │
│   │         배포 후 안전망                │                      │
│   │  롤백 전략 + Feature Flag + 모니터링  │                      │
│   └──────────────────────────────────────┘                      │
│                                                                  │
│   호스팅 플랫폼:                                                 │
│   · CSR 앱 → Vercel / Netlify / S3+CloudFront                  │
│   · SSR 앱 → Vercel / Docker + ECS / Cloud Run                 │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
정적 호스팅 + CDN 동작 방식:
  ...
  빌드 →  dist/ 폴더 생성
          ├── index.html
          ├── assets/vendor-abc123.js  (외부 라이브러리)
          ├── assets/app-def456.js     (앱 코드)
          └── assets/style-ghi789.css

  배포 → CDN 엣지 서버에 파일 전파
          ├── 서울 엣지 서버 (ap-northeast-2)
          ├── 도쿄 엣지 서버 (ap-northeast-1)
          ├── 뉴욕 엣지 서버 (us-east-1)
          └── 런던 엣지 서버 (eu-west-2)

  사용자 요청 → 가장 가까운 엣지 서버가 응답
  → 전 세계 어디서나 빠른 로딩 ★
```
...
개념 간 관계:
  ...
  Feature Flag
     │
     ├── 배포와 릴리스 분리: 코드 배포 ≠ 사용자 노출
     ├── 점진적 롤아웃: 1% → 10% → 50% → 100%
     └── 즉각 비활성화: 플래그 OFF → 재배포 불필요 ★

  롤백
     │
     ├── Vercel Promote: 이전 배포를 프로덕션으로 승격
     ├── Git Revert: main 되돌리기 → CI/CD 재실행
     └── Docker Tag: 이전 이미지 버전으로 재배포
...
CSR (Vite + React Router):
  ...
  배포: 정적 호스팅 (CDN에 파일 업로드)
    · Vercel, Netlify, AWS S3 + CloudFront
    · 서버 로직 불필요
    · 가장 저렴, 가장 빠름, 가장 단순

  ⚠️ SPA 라우팅 설정 필요:
    모든 경로 → index.html로 리다이렉트 (Fallback)
    /products/42 직접 접근 시 서버가 index.html 반환해야 함


SSR (Next.js):

  빌드 결과: .next/ 폴더 (서버 + 정적)
    · 서버 코드 (SSR, API Routes, Server Actions)
    · 정적 페이지 (SSG)
    · 클라이언트 번들

  배포: Node.js 서버 필요
    · Vercel (가장 간편 — Next.js와 같은 회사)
    · Docker 컨테이너 → AWS ECS, GCP Cloud Run 등
    · 정적 호스팅만으로는 불가! (서버가 필요)

  또는: 정적 내보내기 (output: 'export')
    · next.config.js: output: 'export'
    · SSR/SSG/Server Actions 사용 불가
    · CSR과 동일하게 정적 호스팅 가능
```
...
├──────────────────┼──────────────┼──────────────┼──────────────────┤
│  무료 티어       │  넉넉        │  넉넉        │  12개월 프리 티어 │
│  Git 연동 배포   │  자동 ★      │  자동 ★      │  수동/CI 필요    │
│  Preview 배포    │  PR마다 자동 ★│  PR마다 자동 │  직접 구성       │
│  CDN             │  글로벌 ★    │  글로벌      │  CloudFront      │
│  SPA Fallback    │  자동        │  자동        │  수동 설정       │
│  커스텀 도메인   │  간편        │  간편        │  Route 53 연동   │
│  Next.js 지원    │  최고 ★      │  좋음        │  직접 구성       │
│  서버리스 함수   │  Edge/Node   │  Netlify Fn  │  Lambda          │
│  학습 곡선       │  매우 낮음 ★ │  낮음        │  높음            │
│  제어 수준       │  낮음        │  낮음        │  높음 ★          │
│  엔터프라이즈    │  유료        │  유료        │  직접 구성       │
├──────────────────┼──────────────┼──────────────┼──────────────────┤
│  적합한 경우     │  Next.js 앱  │  Gatsby, SPA │  대규모/엔터프라  │
│                  │  빠른 배포   │  JAMstack    │  이즈, 커스텀 요구│
│                  │  개인/스타트업│  개인/소규모 │  기업/대규모     │
└──────────────────┴──────────────┴──────────────┴──────────────────┘
```
