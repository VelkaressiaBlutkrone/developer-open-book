...
=====================================================================
  ...
  [테스트 피라미드]
                      /\
                     /  \
                    / E2E \ ← 수 적음, 비용 높음, 신뢰도 최고
                   /  (5%) \
                  /──────────\
                 /  통합 테스트 \ ← 중간
                /    (25%)      \
               /──────────────────\
              /   단위/컴포넌트 테스트 \ ← 수 많음, 비용 낮음, 빠름
             /        (70%)           \
            /──────────────────────────\
  ...
  2004: Selenium ──────────────────────── (기초 확립)
           │
  2017: Cypress ───────────────────────── (DX 혁신, 브라우저 내부 실행)
           │
  2020: Playwright ────────────────────── (크로스 브라우저, 병렬화, 현재 주류)

  [E2E 테스트 역할 분담]

  컴포넌트 테스트 (RTL)     통합 테스트 (RTL+MSW)     E2E 테스트 (Playwright)
  ─────────────────         ──────────────────         ──────────────────────
  "버튼이 클릭되는가"        "폼 제출이 API를 호출하는가"  "로그인→대시보드가 동작하는가"
  "에러 상태가 표시되는가"    "인증 상태가 올바른가"        "결제 흐름이 끝까지 동작하는가"
  단일 컴포넌트 범위          페이지 단위 범위              전체 앱 범위
...
│  다루는 것                                               │
│  · E2E 테스트의 위치와 역할                              │
│  · Playwright 핵심 API와 사용법                          │
│  · Critical User Journey(CUJ) 식별과 테스트              │
│  · 테스트 종류별 트레이드오프 분석                        │
│  · CI/CD에서의 테스트 자동화                             │
│  · 테스트 유지보수 전략                                   │
│  · 프로젝트 규모별 테스트 전략                           │
