...
StatefulWidget 구조
──────────────────────────────────────────────────────
  StatefulWidget (불변, 자주 재생성)
       ↓ createState() 한 번만 호출
  State 객체 (가변, Element와 함께 살아남음)
       ↓ build()
  Widget Tree 반환
──────────────────────────────────────────────────────
Widget은 매 rebuild마다 새로 만들어져도,
State 객체는 동일하게 유지된다.
```
...
```
[Widget이 트리에 처음 추가될 때]
         ↓
① createState()      — Widget 클래스에서 호출, State 객체 생성
         ↓
② initState()        — State 초기화, 단 한 번만 실행
         ↓
③ didChangeDependencies() — InheritedWidget 의존성 초기 설정
         ↓
④ build()            — UI 구성, 여러 번 호출됨

[부모 Widget이 재빌드될 때]
         ↓
⑤ didUpdateWidget()  — 새 Widget 설정값 전달 시 호출
         ↓
④ build()            — UI 재구성
...
[Widget이 트리에서 제거될 때]
         ↓
⑥ deactivate()       — 트리에서 일시 제거 (이동 시에도 호출)
         ↓
