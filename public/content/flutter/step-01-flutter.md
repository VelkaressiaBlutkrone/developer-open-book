...
Flutter 아키텍처
    ...
    │       ├── Material / Cupertino   ← 플랫폼 디자인 시스템
    │       ├── Widgets                ← UI 컴포넌트 추상화
    │       ├── Rendering              ← 레이아웃·페인팅 계산
    │       ├── Animation / Painting
    │       └── Foundation / Gestures  ← 기반 유틸리티
    │
    ├── 2. Engine Layer (C++)
    │       ├── Skia / Impeller        ← 렌더링 엔진 (픽셀 그리기)
    │       ├── Dart Runtime           ← JIT(개발) / AOT(배포)
    │       ├── Text Layout            ← 텍스트·폰트 처리
    │       └── dart:ui                ← Framework → Engine 진입점
    │
...
[개발자가 작성하는 Dart 코드]
          ↓  Widget / State
[Framework Layer]  — UI 논리, 레이아웃 계산, 그리기 명령 생성
          ↓  dart:ui Canvas API
[Engine Layer]     — 명령을 받아 GPU에서 픽셀로 렌더링
          ↓  OS 창·뷰에 픽셀 전달
[Embedder Layer]   — 플랫폼 창 관리, 입력 이벤트 수신
          ↓
[화면 출력]
```
...
```
┌──────────────────────────────────────────┐
│        Material        │   Cupertino     │  ← Google / Apple 디자인 시스템
├──────────────────────────────────────────┤
│                Widgets                   │  ← StatelessWidget, StatefulWidget
├──────────────────────────────────────────┤
│              Rendering                   │  ← RenderObject 트리, 레이아웃 알고리즘
├──────────────────────────────────────────┤
│   Animation   │   Painting   │  Gestures │  ← 애니메이션, Canvas 래핑, 터치 인식
├──────────────────────────────────────────┤
│          Foundation  /  Services         │  ← ChangeNotifier, ValueNotifier 등
└──────────────────────────────────────────┘
```
...
```
[사용자 입력 발생 / setState 호출 / 애니메이션 tick]
                      ↓
┌─────────────────────────────────────────────────────┐
│ ① Build Phase     Widget Tree 재구성                │ ← Framework (Dart)
│                   변경된 Widget만 선택적 재빌드      │
├─────────────────────────────────────────────────────┤
│ ② Layout Phase    RenderObject 크기·위치 계산        │ ← Framework (Dart)
│                   Box Constraints 전파               │
├─────────────────────────────────────────────────────┤
│ ③ Paint Phase     RenderObject → Canvas 명령 생성   │ ← Framework (Dart)
│                   그리기 순서(z-order) 결정          │
├─────────────────────────────────────────────────────┤
│ ④ Composite Phase  레이어 트리 합성                  │ ← Engine (C++)
│                    RepaintBoundary로 레이어 분리      │
├─────────────────────────────────────────────────────┤
│ ⑤ Rasterize Phase GPU가 픽셀로 변환                  │ ← Skia / Impeller
│                    셰이더 실행, 텍스처 처리           │
└─────────────────────────────────────────────────────┘
                      ↓
               [화면에 표시]
...
```
Cross Platform 렌더링 전략

  ① 웹뷰 방식 (Cordova, Ionic)
     JS/HTML/CSS → 브라우저 엔진이 렌더링
     장점: 웹 기술 재사용
     단점: 성능 낮음, Native 룩앤필 구현 어려움

  ② 브리지 방식 (React Native)
     JS 코드 ──[Bridge]──▶ Native UI 컴포넌트
     장점: 플랫폼 Native UI 사용, 웹 개발자 친화적
     단점: Bridge 직렬화 오버헤드, 플랫폼별 UI 차이 존재

  ③ 자체 렌더링 방식 (Flutter)
     Dart 코드 ──▶ Engine(Skia/Impeller) ──▶ Canvas에 직접 픽셀 그리기
     장점: 플랫폼 무관 완전한 UI 제어, 일관된 성능
     단점: Native UI 컴포넌트 미사용, 번들 크기 증가
```
