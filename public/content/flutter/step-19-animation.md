...
애니메이션의 UX 역할
──────────────────────────────────────────────────────
  ① 공간 인지      카드가 확장되면 "이 화면이 그 카드에서 왔다"
  ② 피드백 제공    버튼이 눌릴 때 눌리는 느낌 → 동작 확인
  ③ 시선 유도      중요한 변경사항을 부드럽게 강조
  ④ 상태 전이      로딩 → 완료 전환을 매끄럽게 표현
  ⑤ 브랜드 개성    앱만의 고유한 움직임 언어 표현
──────────────────────────────────────────────────────
```
...
```
명시적(Explicit) 애니메이션           암시적(Implicit) 애니메이션
──────────────────────────────        ──────────────────────────────
AnimationController 직접 제어          상태 변경 → 자동 전환
시작·정지·반복·방향 수동 조작           setState()로 값 변경하면 알아서 애니메이션
복잡한 커스텀 효과에 적합               단순 상태 변화 UI에 적합
코드량 많음                            코드량 적음

예: 로딩 스피너, 복잡한 시퀀스 애니메이션   예: 버튼 크기 변화, 색상 전환, 패딩 변화
```
...
──────────────────────────────────────────────────────
  ...
  Curves.easeIn          ────────────████  처음 느리고 나중 빠름
  Curves.easeOut         ████────────────  처음 빠르고 나중 느림
  Curves.easeInOut       ────████████────  양끝 느리고 가운데 빠름
  Curves.bounceOut       ████ 퉁퉁퉁       도착 후 바운스
  Curves.elasticOut      ████ 탄성         도착 후 탄성 반응
  Curves.decelerate      ████────────────  빠르게 시작해 천천히 멈춤
──────────────────────────────────────────────────────
```
