...
│  │  수동 패칭의 7가지 한계                    │                  │
│  │  (이 Step의 핵심 학습 결과)               │                  │
│  │  Race Condition / 캐싱 부재 / Stale Data │                  │
│  │  Waterfall / 에러 재시도 / 낙관적 업데이트│                  │
│  │  보일러플레이트                           │                  │
│  └──────────────────────────────────────────┘                  │
│         │                                                       │
│         ▼                                                       │
│    Step 23: TanStack Query가 이 모든 문제를 해결                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · REST API 기초 (리소스, HTTP 메서드, 상태 코드)       │
│  · fetch API 상세 사용법                                │
│  · Axios의 특징과 fetch와의 비교                        │
│  · React에서의 데이터 패칭 상태 관리 패턴               │
│  · CRUD 구현 (GET/POST/PUT/DELETE)                     │
│  · Race Condition, Waterfall 등 수동 패칭의 문제점      │
│  · 캐싱 부재, 중복 요청, 에러 재시도 문제               │
│  · useFetch Custom Hook의 한계 분석                    │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
...
│  · 현재 선택된 탭                 · 댓글                      │
│                                                               │
│  도구: useState, useReducer       도구: useEffect + fetch     │
│        Context, Zustand                 TanStack Query (Step 23)│
│                                         RSC (Step 20~21)      │
│                                                               │
│  핵심: 이 두 종류의 State를 같은 도구로 관리하면              │
│        코드가 복잡해진다 → 분리가 핵심!                       │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
REST URL 설계 원칙
  ...
  /users/42/posts     → 42번 사용자의 게시글 컬렉션
...
├──────────┼───────────────┼──────────────────┼───────────────┤
│  Create  │  POST         │  POST /users      │  사용자 생성  │
│  Read    │  GET          │  GET /users       │  목록 조회    │
│          │  GET          │  GET /users/42    │  개별 조회    │
│  Update  │  PUT          │  PUT /users/42    │  전체 교체    │
│          │  PATCH        │  PATCH /users/42  │  일부 수정    │
│  Delete  │  DELETE       │  DELETE /users/42 │  삭제         │
└──────────┴───────────────┴──────────────────┴───────────────┘
```
...
```
2xx 성공:
  200 OK           → 요청 성공 (GET, PUT, PATCH, DELETE)
...
fetch의 특이한 동작들
  ...
  1. HTTP 에러(4xx, 5xx)에서 reject되지 않는다!
     fetch는 네트워크 에러에서만 reject된다.
     400, 404, 500 등은 "성공한 응답"으로 취급된다.
     → response.ok를 반드시 확인해야 한다!

     // ❌ 에러를 놓치는 코드
     const data = await fetch('/api/users').then(r => r.json());
     // 404여도 data에 에러 객체가 들어올 뿐, 예외가 발생하지 않음!
  ...
  2. response.json()도 Promise다 (await 필요!)
     const response = await fetch(url);  // 1번째 await
     const data = await response.json(); // 2번째 await

  3. 요청 취소: AbortController 사용
...
```
┌──────────────────┬───────────────────────┬───────────────────────┐
│                  │  fetch (내장)          │  Axios (라이브러리)    │
├──────────────────┼───────────────────────┼───────────────────────┤
│  설치            │  불필요 (브라우저 내장) │  npm install axios    │
│  JSON 변환       │  수동 (r.json())       │  자동 (data 속성)     │
│  에러 처리       │  ok 체크 필수          │  HTTP 에러도 reject ★ │
│  요청 취소       │  AbortController       │  CancelToken/Abort    │
│  인터셉터        │  없음                 │  있음 ★               │
│  기본 설정       │  없음                 │  baseURL, headers 등  │
│  요청/응답 변환  │  수동                 │  자동 (transformers)  │
│  진행률 추적     │  ReadableStream       │  onUploadProgress ★   │
│  타임아웃        │  AbortSignal.timeout   │  timeout 옵션        │
│  브라우저 지원   │  모던 브라우저 전부    │  IE11+                │
│  번들 크기       │  0KB                  │  ~13KB                │
│  Node.js 지원   │  18+ (실험적)          │  완전 지원            │
└──────────────────┴───────────────────────┴───────────────────────┘
```
...
```
이 패턴의 문제점 (다음 섹션에서 상세히 분석)

  1. 서버 데이터를 로컬 State에 "복사"해야 한다
     · 두 곳에 같은 데이터 → 동기화 문제
     · "어느 것이 진실인가?" — 서버? 로컬?

  2. Mutation 후 수동으로 로컬 State를 업데이트해야 한다
     · 서버 응답과 로컬 업데이트가 일치하는지 보장 어려움
     · 다른 사용자의 변경은 반영되지 않음

  3. 에러 시 롤백이 어렵다
     · 낙관적 업데이트를 직접 구현하면 매우 복잡
```
...
```
같은 데이터를 여러 곳에서 사용할 때

  Header:    GET /api/user/me → 사용자 정보 (아바타, 이름)
  Sidebar:   GET /api/user/me → 같은 사용자 정보!
  Profile:   GET /api/user/me → 또 같은 사용자 정보!

  캐싱 없이: 3개의 동일한 API 요청 → 서버 부하 × 3, 네트워크 비용 × 3
  캐싱 있으면: 1번만 요청, 3곳에서 캐시 공유

  수동 구현 시:
    · 전역 캐시 객체를 직접 관리해야 한다
    · 캐시 무효화 로직을 직접 구현해야 한다
    · "이 데이터가 오래되었는가?" 판단을 직접 해야 한다
    → 사실상 TanStack Query를 직접 만드는 것과 같다
```

#### 문제 3: 데이터 동기화(Stale Data)

```
시나리오: 사용자 A가 상품 목록을 보는 동안
         다른 사용자 B가 새 상품을 추가했다

  수동 패칭: 사용자 A는 페이지를 새로고침하지 않는 한
             새 상품을 볼 수 없다

  TanStack Query: windowFocus 시 자동 리패칭, 주기적 폴링 등으로
                  최신 데이터를 자동으로 가져온다

  수동 구현 시:
    · setInterval로 주기적 패칭? → 비효율, 정리 관리 복잡
    · focus 이벤트 리스너? → 직접 구현 + 정리 필요
    · WebSocket? → 추가 인프라 필요
```
...
부모-자식 컴포넌트의 데이터 의존성
  ...
  function UserPosts({ user }) {
    const { data: posts } = useFetch(`/api/users/${user.id}/posts`);
    // 부모 패칭(200ms) + 자식 패칭(200ms) = 400ms (직렬!)

    if (!posts) return <Spinner />;
    return posts.map(p => <PostCard key={p.id} post={p} />);
  }

  // 이상적: 두 요청을 병렬로 실행하면 max(200, 200) = 200ms
  // 현실: useEffect는 렌더링 후 실행 → 부모 완료 후에야 자식 패칭 시작
```
...
네트워크가 불안정할 때
  ...
  수동: 요청 실패 → 에러 표시 → 사용자가 "다시 시도" 클릭

  라이브러리: 실패 시 자동 재시도 (exponential backoff)
    1차 시도 실패 → 1초 후 재시도
    2차 시도 실패 → 2초 후 재시도
    3차 시도 실패 → 에러 표시
...
좋아요 버튼을 누를 때
  ...
  일반 업데이트:
    클릭 → 서버 요청 (300ms) → 성공 → UI 반영
    사용자: "왜 반응이 느리지?"

  낙관적 업데이트:
    클릭 → UI 즉시 반영 + 서버 요청 → 성공 시 확정, 실패 시 롤백
    사용자: "즉시 반응하네!"
  ...
  수동 구현의 복잡성:
    ...
    2. 낙관적으로 State 업데이트
    3. 서버 요청
    4. 성공: 서버 응답으로 State 확정
    5. 실패: 백업한 State로 롤백 + 에러 표시
    6. 동시에 여러 낙관적 업데이트가 발생하면? → 매우 복잡
```
...
│  수동 패칭의 7가지 한계                                       │
│                                                               │
│  1. Race Condition — 응답 순서 불일치                         │
│  2. 캐싱 부재 — 같은 데이터 중복 요청                        │
│  3. 데이터 동기화 — Stale Data 감지/갱신 불가                │
│  4. Waterfall — 직렬 요청으로 인한 느린 로딩                 │
│  5. 에러 재시도 — 네트워크 불안정 시 수동 처리               │
│  6. 낙관적 업데이트 — 수동 구현이 매우 복잡                  │
│  7. 보일러플레이트 — 매 패칭마다 loading/error/data 반복      │
│                                                               │
│  이 모든 문제를 해결하는 것이 TanStack Query (Step 23)       │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
```
요구사항:
  · JSONPlaceholder API 사용 (https://jsonplaceholder.typicode.com)
  · 게시글 CRUD:
    - GET /posts → 게시글 목록 조회
  ...
  · 에러 핸들링 (response.ok 체크, try/catch)
...
```
요구사항:
  · API 인스턴스 생성 (baseURL, timeout, 기본 headers)
  · 요청 인터셉터: 모든 요청에 타임스탬프 헤더 추가
  · 응답 인터셉터: 응답 시간 로깅 (요청 시작~응답 도착)
  · 에러 인터셉터: 에러 코드별 다른 메시지 변환
    - 401 → "로그인이 필요합니다"
    - 403 → "접근 권한이 없습니다"
    - 404 → "요청한 데이터를 찾을 수 없습니다"
    - 500 → "서버 오류가 발생했습니다"
  · 이 인스턴스를 사용하여 실습 1의 CRUD를 리팩토링
```
...
```
실험 A — Race Condition:
  · 사용자 프로필 페이지: userId를 빠르게 전환
  · AbortController 없이 구현 → 잘못된 데이터 표시 관찰
  · AbortController 추가 → 문제 해결 확인
  · 분석: "매번 이 코드를 작성해야 하는가?"

실험 B — 캐싱 부재:
  · 3개 컴포넌트(Header, Sidebar, Profile)에서 같은 /api/user/me 호출
  · DevTools Network 탭에서 동일 요청 3번 확인
  · 분석: "이 중복을 어떻게 제거할 수 있는가?"

실험 C — Waterfall:
  · 부모: GET /api/user/:id → 200ms
  · 자식: GET /api/user/:id/posts → 200ms (부모 완료 후 시작)
  · 총 시간: 400ms
  · Promise.all로 병렬화 시도 → 의존성 때문에 불가능한 이유 분석
```
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. REST API = 리소스(URL) + HTTP 메서드(동사) + 상태 코드    │
│     → GET(조회), POST(생성), PUT/PATCH(수정), DELETE(삭제)   │
│     → 2xx(성공), 4xx(클라이언트 에러), 5xx(서버 에러)        │
│                                                               │
│  2. fetch는 HTTP 에러에서 reject되지 않는다!                  │
│     → response.ok 확인 필수                                  │
│     → response.json()도 await 필요                           │
│     → AbortController로 요청 취소                            │
│                                                               │
│  3. Axios는 fetch 위에 편의 기능을 추가한다                    │
│     → 인터셉터, baseURL, 자동 JSON 변환, HTTP 에러 reject    │
│     → 인증 토큰, 에러 변환, 로깅 등에 유용                   │
│     → 번들 크기(~13KB) vs 편의성 트레이드오프                 │
│                                                               │
│  4. Server State와 Client State를 구분해야 한다               │
│     → Server State: 서버가 소유, 비동기, stale 가능          │
│     → Client State: 클라이언트가 소유, 동기, 항상 최신        │
│     → 같은 도구로 관리하면 복잡성 증가                        │
│                                                               │
│  5. 수동 패칭은 매 컴포넌트마다 보일러플레이트를 반복한다      │
│     → loading/error/data State + useEffect + Cleanup          │
│     → useFetch Custom Hook으로 추출 가능하지만 한계가 있다    │
│                                                               │
│  6. 수동 패칭의 7가지 한계                                    │
│     → Race Condition, 캐싱 부재, Stale Data                  │
│     → Waterfall, 에러 재시도, 낙관적 업데이트, 보일러플레이트 │
│     → 이 문제들을 해결하는 것이 TanStack Query (Step 23)     │
│                                                               │
│  7. 병렬 요청: Promise.all / Promise.allSettled               │
│     → 독립적 데이터: Promise.all로 병렬 패칭                 │
│     → 부분 실패 허용: Promise.allSettled                     │
│     → 의존적 데이터: Waterfall이 불가피 (라이브러리로 최적화) │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
