...
웹 성능 최적화 전체 개념 지도
  ...
  │                  사용자 체감 성능의 3대 축                    │
  │                                                              │
  │  LCP (로딩)          INP (반응성)        CLS (안정성)        │
  │  ─────────────       ──────────────      ──────────────      │
  │  코드 분할           useTransition       이미지 크기 명시    │
  │  이미지 최적화       Virtualization      Skeleton UI         │
  │  번들 축소           메모이제이션         폰트 최적화         │
  │  SSR/SSG             DOM 노드 감소       레이아웃 예약        │
  │         │                   │                   │           │
  │         └───────────────────┴───────────────────┘           │
  │                             │                               │
  │                    측정 도구 (Lighthouse)                    │
  ...
  │                             │                               │
  │              ┌──────────────┴──────────────┐                │
  │         번들 분석                      React Profiler        │
  │    (rollup-plugin-visualizer)      (컴포넌트 성능 측정)      │
  └─────────────────────────────────────────────────────────────┘
```
...
│  다루지 않는 것                                          │
│  · 서버 최적화 (CDN, 캐싱 헤더, 압축)                   │
│  · SSR/SSG 최적화 (Step 19~21에서 학습)                 │
│  · React Compiler (Step 14에서 개요 학습)               │
│  · Web Worker를 활용한 오프스레드 계산                    │
└─────────────────────────────────────────────────────────┘
```
...
│                                                              │
│  LCP (로딩 속도)           INP (반응성)         CLS (안정성)  │
│  ─────────────             ─────────            ──────────── │
│  가장 큰 콘텐츠가          사용자 상호작용      레이아웃      │
│  표시되는 시간             후 화면 반영 시간     이동 정도     │
│                                                              │
│  좋음: ≤ 2.5s             좋음: ≤ 200ms        좋음: ≤ 0.1  │
│  보통: ≤ 4.0s             보통: ≤ 500ms        보통: ≤ 0.25 │
│  나쁨: > 4.0s             나쁨: > 500ms        나쁨: > 0.25 │
│                                                              │
│  영향 요소:               영향 요소:           영향 요소:     │
│  · 큰 이미지 로드          · 무거운 JS 실행    · 크기 미지정   │
│  · 서버 응답 시간          · 긴 렌더링         · 이미지 로드   │
│  · 렌더 차단 리소스        · 메인 스레드 점유  · 동적 콘텐츠   │
│  · 큰 JS 번들              · 과도한 DOM 조작   · 폰트 로드     │
│                                                              │
│  Google 검색 랭킹에 직접 영향을 미치는 지표! ★               │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
코드 분할 없이:
  ...
  사용자가 홈 페이지만 방문하는데
  전체 앱의 모든 코드가 한 번에 다운로드된다

  bundle.js = 800KB
    ├── 홈 페이지 코드: 50KB      ← 지금 필요
    ├── 상품 페이지 코드: 150KB   ← 아직 불필요
    ├── 대시보드 코드: 200KB      ← 아직 불필요
    ├── 관리자 코드: 100KB        ← 대부분의 사용자에게 불필요!
    ├── 차트 라이브러리: 200KB    ← 대시보드에서만 사용
    └── 기타: 100KB

  사용자는 50KB만 필요한데 800KB를 다운로드한다!
  → LCP 지연, TTI 지연
```
...
코드 분할 후:
  ...
  필요 시 추가 로드:
    products-chunk.js = 150KB    ← /products 방문 시
    dashboard-chunk.js = 400KB   ← /dashboard 방문 시 (차트 포함)
    admin-chunk.js = 100KB       ← /admin 방문 시

  결과:
    · 초기 로딩: 800KB → 150KB (81% 감소!)
    · 페이지 전환 시 해당 청크만 추가 로드
    · 사용하지 않는 페이지의 코드는 영원히 로드 안 함
```
...
번들 크기를 "측정"하는 도구
  ...
  1. rollup-plugin-visualizer (Vite 권장)
     ...
     → npm run build 후 번들 시각화 HTML 생성
  ...
  분석에서 확인할 것:
    · 가장 큰 모듈은 무엇인가?
    · 사용하지 않는 라이브러리가 번들에 포함되어 있는가?
    · 특정 페이지에서만 사용하는 라이브러리가 메인 번들에 있는가?
```

#### 번들 크기 줄이기 전략

```
전략 1: 무거운 라이브러리를 동적 import

  // ❌ 정적 import — 항상 메인 번들에 포함
  import { Chart } from 'chart.js';

  // ✅ 동적 import — 차트를 사용하는 페이지에서만 로드
  const ChartComponent = lazy(() => import('./components/Chart'));


전략 2: 가벼운 대안 라이브러리 사용

  · moment.js (300KB) → date-fns (tree-shakable) 또는 dayjs (2KB)
  · lodash (70KB) → lodash-es (tree-shakable) 또는 개별 import
  · axios (13KB) → fetch API (0KB, 내장)

  // ❌ lodash 전체 import
  import _ from 'lodash';   // 70KB 전부 번들에!
  _.debounce(fn, 300);

  // ✅ 개별 import (tree-shakable)
  import debounce from 'lodash/debounce';   // debounce만 번들에
  debounce(fn, 300);


전략 3: Tree Shaking 확인

  · ES Module(import/export)을 사용해야 Tree Shaking 가능
  · CommonJS(require/module.exports)는 Tree Shaking 불가
  · package.json의 "sideEffects": false 확인
  · 번들 분석으로 미사용 코드가 제거되었는지 확인


전략 4: 이미지/폰트를 번들 밖으로 분리

  · 이미지를 JS 번들에 포함하지 않는다 (import img X)
  · CDN에서 이미지를 제공한다
  · 폰트를 로컬에 포함하되 display: swap으로 FOIT 방지
```
...
```
이미지는 웹 페이지에서 가장 큰 리소스 (평균 전체 크기의 50%+)

  전략 1: 적절한 포맷 사용
    · WebP: JPEG/PNG 대비 25~35% 작음. 모던 브라우저 전부 지원
    · AVIF: WebP보다 더 작지만 지원 범위가 좁음
    · SVG: 아이콘, 로고 등 벡터 그래픽
    · <picture> 태그로 포맷 폴백 제공

  전략 2: 반응형 이미지
    · srcSet으로 디바이스 해상도에 맞는 크기 제공
    · 모바일에 4K 이미지를 보내지 않는다

  전략 3: 지연 로딩 (Lazy Loading)
    · 뷰포트에 보이지 않는 이미지는 나중에 로드
    · <img loading="lazy" /> — 네이티브 지원
    · Intersection Observer를 활용한 커스텀 구현

  전략 4: 크기 명시로 CLS 방지
    · width, height 또는 aspect-ratio를 항상 지정
    · 이미지 로드 전에 공간이 확보되어 레이아웃 이동 없음

  전략 5: LCP 이미지 우선 로드
    · 가장 큰 콘텐츠가 이미지라면 → fetchpriority="high"
    · <img fetchpriority="high" src="hero.webp" />
    · preload 링크: <link rel="preload" as="image" href="hero.webp">
```
...
사용자가 화면에서 실제로 보는 것:
  · 약 20~30개 항목만 보임!
  · 나머지 9,970개는 화면 밖에 존재
  → "보이지 않는 DOM을 왜 만들어야 하는가?"
```
...
Virtualization의 원리
  ...
  화면에 보이는 영역(viewport) + 약간의 버퍼만 DOM에 렌더링
  나머지는 DOM에 존재하지 않음

  스크롤 위치에 따라 동적으로 DOM을 교체

  전체 10,000개 항목:
    [---보이지 않음---][===보이는 영역===][---보이지 않음---]
                        ↑ 이 부분만 DOM!

  DOM 노드 수: 10,000개 → 약 30~50개 (99.5% 감소!)
  렌더링 시간: 수백 ms → 수 ms
  메모리: 수십 MB → 수 KB
```
...
LCP 개선 전략:
  ──────────────
  1. 이미지 최적화 (WebP, 적절한 크기, preload)
  2. 코드 분할로 초기 JS 번들 축소
  3. SSR/SSG로 서버에서 HTML 제공
  4. render-blocking CSS 최소화 (critical CSS 인라인)
  5. 서버 응답 시간 개선 (CDN, 캐싱)

INP 개선 전략:
  ──────────────
  1. 무거운 JS 실행 최소화 (useMemo, 코드 분할)
  2. useTransition으로 긴급/비긴급 업데이트 분리
  3. Virtualization으로 DOM 노드 수 감소
  4. Long Task 분할 (yield to main thread)
  5. Third-party 스크립트 지연 로드

CLS 개선 전략:
  ──────────────
  1. 이미지/비디오에 width, height 속성 명시
  2. 동적 콘텐츠에 미리 공간 확보 (skeleton, placeholder)
  3. 웹 폰트: font-display: swap + preload
  4. 광고/임베드에 고정 크기 컨테이너
  5. 조건부 렌더링으로 인한 레이아웃 이동 방지
```
...
"무엇을 먼저 최적화해야 하는가?"
  ...
  ┌─────────────────────────────────────────────────────┐
  │  Priority 1: 가장 높은 효과 / 가장 낮은 비용         │
  │                                                      │
  │  · 코드 분할 (React.lazy + 라우트 기반)              │
  │  · 이미지 최적화 (WebP, lazy, 크기 지정)             │
  │  · Tree Shaking 확인 (무거운 라이브러리 제거/대체)   │
  │  → 대부분의 앱에서 가장 큰 개선 효과                 │
  │                                                      │
  ├─────────────────────────────────────────────────────┤
  │  Priority 2: 측정 기반 최적화                        │
  │                                                      │
  │  · Lighthouse로 병목 식별                            │
  │  · 번들 분석으로 큰 모듈 찾기                        │
  │  · React Profiler로 느린 컴포넌트 찾기               │
  │  → "측정 → 식별 → 최적화 → 재측정" 사이클           │
  │                                                      │
  ├─────────────────────────────────────────────────────┤
  │  Priority 3: 상황별 최적화                           │
  │                                                      │
  │  · 대규모 리스트 → Virtualization                    │
  │  · 무거운 계산 → useMemo, useTransition              │
  │  · 빈번한 리렌더링 → React.memo + useCallback       │
  │  → 문제가 실제로 체감될 때만 적용                    │
  │                                                      │
  ├─────────────────────────────────────────────────────┤
  │  Priority 4: 아키텍처 레벨                           │
  │                                                      │
  │  · SSR/SSG 도입 (SEO + 초기 로딩)                   │
  │  · CDN 활용 (정적 자원 캐싱)                        │
  │  · 서비스 워커 (오프라인 캐싱)                       │
  │  → 도입 비용이 높으므로 필요성을 신중히 판단         │
  │                                                      │
  └─────────────────────────────────────────────────────┘

  핵심 원칙:
    1. 측정 없이 최적화하지 않는다 (Step 14 복습)
    2. 가장 높은 효과의 최적화부터 적용한다
    3. 과도한 최적화는 코드 복잡성을 높인다
    4. "충분히 빠르면" 더 최적화하지 않는다
```
...
Before: 단일 번들
  ...
  · 페이지 코드: 200KB
  · 차트 라이브러리: 250KB (대시보드에서만 사용)
  · 날짜 라이브러리: 70KB
  · 기타: 150KB

  초기 로딩: 800KB 다운로드 + 파싱 + 실행
  → FCP: 3.2초, LCP: 4.5초


After: 라우트 기반 코드 분할
  main.js: 150KB (React + 홈 페이지 + 공통)
  products-chunk.js: 120KB (필요 시 로드)
  dashboard-chunk.js: 350KB (차트 포함, 필요 시 로드)
  admin-chunk.js: 80KB (관리자만 로드)

  초기 로딩: 150KB 다운로드 + 파싱 + 실행
  → FCP: 1.1초, LCP: 1.8초

  개선: FCP 66% 감소, LCP 60% 감소
```
...
해결: 이미지에 크기를 지정하여 공간 미리 확보

  // ❌ 크기 미지정 — CLS 발생
  <img src="product.jpg" alt="상품" />

  // ✅ 크기 지정 — CLS 방지
  <img src="product.jpg" alt="상품" width={600} height={400} />

  // ✅ CSS aspect-ratio — 반응형에서도 CLS 방지
  <div style={{ aspectRatio: '16/9', width: '100%' }}>
    <img src="product.jpg" alt="상품" style={{ width: '100%', height: '100%' }} />
  </div>
...
```
과제:
  1. 실습 1의 앱을 빌드하고 로컬 서버로 실행
  2. Lighthouse Performance 측정 (초기 점수 기록)
  3. Opportunities/Diagnostics 확인
  4. 최소 3가지 개선 적용:
     · 이미지 최적화 (WebP, lazy, 크기 지정)
     · CSS/JS 최적화 (코드 분할 확인)
     · 폰트 최적화 (display: swap)
...
```
과제:
  1. rollup-plugin-visualizer 설치 및 설정
  2. npm run build → 번들 시각화 확인
  3. 가장 큰 모듈 3개 식별
  4. 각 모듈에 대해 최적화 방법 결정:
     · 동적 import로 분리?
     · 가벼운 대안으로 교체?
...
분석 보고서:
  | 모듈명    | 크기(전) | 최적화 방법 | 크기(후) |
  |----------|---------|-----------|---------|
  | chart.js | 250KB   | lazy      | 0KB(초기)|
  | ...      | ...     | ...       | ...     |
```
...
```
요구사항:
  · 10,000개 항목의 상품 리스트 생성
  · 일반 렌더링과 가상화 렌더링 두 버전 구현
  · 성능 비교:
    - 초기 렌더링 시간 (Performance.now())
    - 스크롤 시 프레임 레이트 (DevTools Performance)
    - DOM 노드 수 (DevTools Elements)
  · TanStack Virtual 또는 react-window 사용
  · 각 항목: 이미지, 이름, 가격, 설명 (복잡한 UI)
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. Core Web Vitals = 사용자 체감 성능의 3대 지표             │
│     → LCP(≤2.5s): 가장 큰 콘텐츠 표시 시간                  │
│     → INP(≤200ms): 상호작용 반응 시간                        │
│     → CLS(≤0.1): 레이아웃 안정성                             │
│     → Google 검색 랭킹에 직접 영향                           │
│                                                               │
│  2. 코드 분할 = 가장 효과적인 초기 로딩 최적화                │
│     → React.lazy + Suspense로 라우트 기반 분할               │
│     → 초기 번들을 80%+ 줄일 수 있다                          │
│     → 페이지 전환 시 필요한 청크만 추가 로드                  │
│     → 호버 시 prefetch로 체감 지연 제거                      │
│                                                               │
│  3. 번들 분석 → 측정 기반 최적화                              │
│     → rollup-plugin-visualizer로 시각화                      │
│     → 큰 라이브러리: 동적 import 또는 가벼운 대안            │
│     → Tree Shaking: ES Module + sideEffects 확인             │
│                                                               │
│  4. 이미지 최적화 = 가장 큰 리소스 최적화                     │
│     → WebP/AVIF 포맷, 반응형 srcSet                          │
│     → loading="lazy" (LCP 이미지 제외!)                      │
│     → width/height 명시로 CLS 방지                           │
│     → Next.js <Image>가 대부분 자동 처리                     │
│                                                               │
│  5. Virtualization = 대규모 리스트의 유일한 해법              │
│     → 보이는 항목만 DOM에 렌더링                             │
│     → 10,000개 → ~30개 DOM 노드 (99.5% 감소)                │
│     → TanStack Virtual, react-window                         │
│     → 500개+ 항목에서 고려                                   │
│                                                               │
│  6. Lighthouse로 측정 → 개선 → 재측정 사이클                  │
│     → Opportunities: 구체적 개선 제안                        │
│     → Diagnostics: 성능 병목 정보                            │
│     → "충분히 빠르면" 더 최적화하지 않는다                    │
│                                                               │
│  7. 최적화 우선순위                                           │
│     → P1: 코드 분할, 이미지 최적화, Tree Shaking (효과 큼)   │
│     → P2: 측정 기반 병목 해결                                │
│     → P3: 상황별 (가상화, 메모이제이션)                      │
│     → P4: 아키텍처 (SSR/SSG, CDN)                            │
│                                                               │
