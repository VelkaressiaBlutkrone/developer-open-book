...
컴포넌트 설계 패턴 진화 개념 지도
  ...
  │                    패턴의 역사적 진화                         │
  │                                                              │
  │  2015     2017     2019     2021~현재                        │
  │   │        │        │         │                             │
  │  Mixins   HOC    Render    Custom Hook (로직 재사용 표준)    │
  │   │       │      Props       │                             │
  │  (폐기)  (레거시)  (레거시)  Compound Component (UI 유연성) │
  │                              │                             │
  │                             Headless Component (최대 커스텀)│
  │                                                              │
  │  현재 권장 조합:                                             │
  │  · 로직 재사용 → Custom Hook                                │
  │  · UI 구조 유연성 → Compound Component                      │
  │  · 라이브러리 설계 → Headless Component                     │
  │  · 레거시 유지보수 → HOC, Render Props 이해 필요            │
  └─────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · 컴포넌트 분류: Presentational vs Container (현대 해석)│
│  · Compound Component 패턴 ★                            │
│  · Render Props 패턴 (역사와 현재)                      │
│  · HOC(Higher-Order Component) 패턴 (역사와 현재)       │
│  · Headless Component 패턴                              │
│  · 컴포넌트 API 설계 원칙                               │
│  · 패턴의 진화: HOC → Render Props → Hook → Compound   │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · 디자인 시스템 구축 (Step 34)                         │
│  · 스타일링 라이브러리 상세 (Step 34)                   │
│  · 접근성(a11y) 상세 (Step 35)                         │
└─────────────────────────────────────────────────────────┘
```
...
```
React 컴포넌트 재사용 패턴의 진화

  2015~2017: Mixins → HOC
    · Mixins: class 컴포넌트에 로직 주입 (폐기됨)
    · HOC: 컴포넌트를 감싸서 기능 추가 (withRouter, connect)

  2017~2019: HOC → Render Props
    · Render Props: 함수를 Props로 전달하여 렌더링 위임
    · HOC보다 명시적이고 타입 추론이 용이

  2019~현재: Render Props → Custom Hook ★
    · Hook이 로직 재사용의 표준이 됨
    · HOC와 Render Props의 대부분의 사용 사례를 대체
    · 더 간결하고, 합성이 쉽고, 테스트가 용이

  병행 사용: Compound Component + Headless Component
    · Hook과 함께 "UI 구조의 유연성"을 제공하는 패턴
    · 라이브러리 설계에서 핵심적으로 사용
    · Radix UI, Headless UI, React Aria 등

  현재 권장:
    · 로직 재사용 → Custom Hook (Step 16)
    · UI 구조 유연성 → Compound Component
    · 최대 커스터마이즈 → Headless Component
    · 레거시 유지보수 → HOC, Render Props 이해 필요
```
...
컴포넌트 설계 패턴 관계도
  ...
  │  ──────────────────────────────────────────────────────►   │
  │                                                              │
  │  Props 기반    Slot 패턴    Compound         Headless        │
  │  컴포넌트                   Component        Component       │
  │                                                              │
  │  무엇을 보여줄지:                                            │
  │  Props로 지정   영역별 지정  구조 자유 제어   완전 자유       │
  │                                                              │
  │  적합한 상황:                                                │
  │  간단한 UI     레이아웃     복합 UI 컴포넌트  라이브러리 설계 │
  │  빠른 구현     영역 커스텀  Accordion/Tabs   어떤 디자인도   │
  └─────────────────────────────────────────────────────────────┘

  로직 재사용 레이어:
  Custom Hook ──── 모든 패턴과 조합 가능 ────────────────────►
```
...
```
Compound Component의 특징

  1. 내부 상태(openId)가 Context로 자식에게 암묵적으로 공유된다
     · 사용자가 openId를 직접 관리하지 않아도 됨
     · toggle 로직이 캡슐화됨

  2. 자식의 순서와 구성을 사용자가 자유롭게 제어한다
     · Item의 순서를 바꿀 수 있다
     · Header에 아이콘을 추가할 수 있다
     · Panel에 어떤 콘텐츠든 넣을 수 있다
     · 특정 Item만 비활성화할 수 있다

  3. "닷 표기법"(Accordion.Item)으로 관련 컴포넌트를 묶는다
     · import { Accordion } from '...' 하나로 모든 하위 컴포넌트 접근
     · IDE 자동완성: Accordion. 입력하면 Item, Header, Panel 제안

  4. 비교: Props 기반 vs Compound 기반

     // Props 기반 — 유연성 제한
     <Accordion
       items={[
         { id: 'q1', title: 'React란?', content: '라이브러리입니다.' },
         { id: 'q2', title: 'Hooks란?', content: 'API입니다.' },
       ]}
     />
     // → title에 아이콘을 넣고 싶다면? Props 인터페이스 수정 필요!
     // → content에 이미지를 넣고 싶다면? 더 복잡해짐!

     // Compound 기반 — 완전한 유연성
     <Accordion>
       <Accordion.Item id="q1">
         <Accordion.Header id="q1">
           <Icon name="react" /> React란?    ← 아이콘 자유롭게 추가
         </Accordion.Header>
         <Accordion.Panel id="q1">
           <p>라이브러리입니다.</p>
           <img src="react.png" />           ← 이미지도 자유롭게
         </Accordion.Panel>
       </Accordion.Item>
     </Accordion>
```
...
```
Render Props가 해결하던 문제:
  · 상태 로직의 재사용 (여러 컴포넌트에서 마우스 위치 추적)
  · 렌더링의 유연성 (사용자가 UI를 결정)

Custom Hook이 더 나은 이유:
  · 중첩이 없다 (Render Props는 콜백 중첩 → "Callback Hell")
  · 합성이 자연스럽다 (Hook을 여러 개 호출)
  · 타입 추론이 쉽다
  ...
  function MyComponent() {
    ...
    return <p>{mouse.x} / {size.width}</p>;
  }

현재 Render Props가 여전히 유용한 경우:
  · 라이브러리가 "렌더링 커스터마이즈"를 제공할 때
  · Downshift, React Table 등 Headless 라이브러리
  · Compound Component 내부에서 부분적으로 사용
```
...
가장 제한적 ──────────────────────────── 가장 유연
  ...
  Props 기반         Slot 패턴          Compound          Headless
  <Button            <Card              <Tabs>            useCombobox()
    label="클릭"       header={...}       <Tabs.List>      · UI 완전 제어
    icon="star"        footer={...}       <Tabs.Tab>       · 스타일 완전 제어
    variant="primary"  >내용<            </Tabs>           · 접근성 자동
  />                  </Card>

  · 간단한 컴포넌트   · 레이아웃 영역   · 복합 UI          · 라이브러리 설계
  · 제한된 커스텀     · 영역별 커스텀   · 구조 커스텀       · 최대 커스텀
  · 빠른 구현         · 적당한 유연성   · 상태 공유         · 복잡한 구현


선택 기준:
  · 사용처가 정해져 있고 변경 가능성이 낮다 → Props 기반
  · 레이아웃의 특정 영역만 커스텀이 필요하다 → Slot 패턴
  · 복합 UI의 구조와 내용을 자유롭게 제어해야 한다 → Compound
  · 라이브러리로 배포하여 어떤 디자인 시스템에도 적용해야 한다 → Headless
```
...
실제 라이브러리들이 사용하는 패턴
  ...
  Compound Component:
    ...
    · Headless UI: Listbox, Menu, Popover
    · Chakra UI: Tabs, Accordion, Menu

  Headless (Hook 기반):
    · TanStack Table: useReactTable() → 테이블 로직만
    · Downshift: useCombobox(), useSelect()
    · React Aria: useButton(), useTextField(), useDialog()

  Props 기반 (간단한 컴포넌트):
    · Material UI: <Button variant="contained" color="primary">
    · Ant Design: <Button type="primary" size="large">

  대부분의 성숙한 UI 라이브러리는 여러 패턴을 혼합하여 사용:
    · 간단한 컴포넌트 → Props 기반
    · 복합 UI → Compound Component
    · 최대 유연성 → Headless Hook
```
...
```
아래 8가지 시나리오에서 적합한 패턴을 선택하고 근거를 제시하라.

1. 앱 전체에서 사용하는 기본 Button 컴포넌트
2. 복잡한 데이터 테이블 (정렬, 필터, 페이지네이션)
3. 모달/다이얼로그 (Trigger + Overlay + Content + Actions)
4. 인증 상태에 따른 페이지 접근 제어
5. 다크 모드 토글 기능을 여러 컴포넌트에 적용
6. 자동완성 검색 입력 (어떤 디자인 시스템에서도 사용 가능해야 함)
7. 페이지 레이아웃 (헤더, 사이드바, 콘텐츠, 푸터 영역)
8. 스텝 위자드 (Step 1 → Step 2 → Step 3 → 완료)

선택지: Props 기반 / Slot 패턴 / Compound Component /
        Headless Hook / Custom Hook / HOC
```
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. 패턴의 진화: HOC → Render Props → Hook → Compound       │
│     → 로직 재사용: Custom Hook이 현재 표준 (Step 16)         │
│     → UI 구조 유연성: Compound Component가 핵심 패턴         │
│     → HOC/Render Props: 레거시 이해용 + 특수 사례에서만 사용 │
│                                                               │
│  2. Compound Component = 여러 컴포넌트가 상태를 공유하며 동작 │
│     → Context로 내부 상태 암묵적 공유                        │
│     → 닷 표기법(Parent.Child)으로 네임스페이스 구성          │
│     → 사용자가 구조와 내용을 완전히 제어                     │
│     → Accordion, Tabs, Dropdown, Modal 등에 적합             │
│                                                               │
│  3. Headless Component = UI 없이 로직만 제공                  │
│     → Custom Hook으로 구현                                   │
│     → Props Getter 패턴: 접근성 + 로직을 요소에 적용         │
│     → 어떤 디자인 시스템에도 적용 가능                       │
│     → 라이브러리 설계의 핵심 패턴                            │
│                                                               │
│  4. 유연성의 단계 (Flexibility Ladder)                        │
│     → Props 기반 < Slot < Compound < Headless                │
│     → 단순한 컴포넌트 → Props 기반                          │
│     → 복합 UI → Compound                                    │
│     → 라이브러리 수준 → Headless                            │
│                                                               │
│  5. 컴포넌트 API 설계 원칙                                    │
│     → 합리적 기본값 + 탈출구 (80/20 원칙)                    │
│     → 일관된 Props 네이밍 (is-, on-, variant 등)             │
│     → "Pit of Success": 쉬운 방법 = 올바른 방법              │
│                                                               │
│  6. Presentational/Container는 현대에서 Hook으로 대체          │
│     → Custom Hook = Container 역할 (로직)                    │
│     → 컴포넌트 = Presentational 역할 (UI)                   │
│     → 폴더 분리보다 "관심사 분리 원칙"을 유지하는 것이 중요  │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
