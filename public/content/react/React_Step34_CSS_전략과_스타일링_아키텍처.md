...
```
        ...
        │                   │                   │
 ┌──────▼──────┐    ┌───────▼───────┐   ┌──────▼──────┐
 │  스코프 전략 │    │  디자인 시스템 │   │  빌드 방식  │
 └──────┬──────┘    └───────┬───────┘   └──────┬──────┘
        │                   │                   │
  ┌─────┼─────┐       ┌─────┼─────┐       ┌────┼────┐
  │     │     │       │     │     │       │    │    │
전역  로컬  인라인   토큰  테마  컴포넌트 빌드타임 런타임 제로런타임
 CSS Module style    색상 다크모드 라이브러리  CSS   CSS-in-JS  Tailwind
                                                           CSS Modules
        │                   │                   │
        └───────────────────┼───────────────────┘
                            │
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · 스타일링 방식 5종 비교 (Vanilla/Modules/Tailwind/    │
│    CSS-in-JS/utility)                                   │
│  · Tailwind CSS 핵심 (유틸리티 퍼스트, 커스텀 설정)     │
│  · CSS Modules (로컬 스코프, 합성)                      │
│  · 디자인 토큰과 테마 시스템                             │
│  · 반응형 디자인 패턴                                    │
│  · 다크 모드 구현 전략                                   │
...
전통적 ←──────────────────────────────────→ 모던
  ...
  Vanilla CSS      CSS Modules     Tailwind CSS     CSS-in-JS
  + BEM            + PostCSS       + 유틸리티       + styled-components
  ──────────       ──────────      ──────────       ──────────
  · 프레임워크 무관 · 로컬 스코프    · 유틸리티 클래스 · JS에서 CSS 작성
  · 전역 스코프    · 빌드 타임 변환 · 빌드 타임 생성  · 런타임 생성
  · 네이밍 규칙 필요· 일반 CSS 활용  · CSS 파일 불필요 · 동적 스타일 강점
  · 제로 런타임    · 제로 런타임    · 제로 런타임     · 런타임 비용!
```
...
│  전역 CSS (BEM):                                          │
│    .card { ... }          .card__title { ... }           │
│    ↓ 모든 .card에 적용 → 충돌 가능성 상존               │
│                                                           │
│  CSS Modules:                                             │
│    .card { ... }  →  빌드 후  .Card_card_a1b2 { ... }   │
│    ↓ 자동 스코프 → 다른 .card와 절대 충돌 안 함 ★        │
│                                                           │
│  Tailwind CSS:                                            │
│    className="bg-white rounded-lg p-4 shadow"            │
│    ↓ 클래스 이름 없음 → 충돌 자체가 불가능 ★             │
│                                                           │
│  CSS-in-JS (styled-components):                          │
│    const Card = styled.div`...`                          │
│    ↓ 런타임에 고유 클래스 생성 → 스코프 보장             │
│    ↓ 단: 렌더링마다 CSS 문자열 생성 비용 발생            │
└──────────────────────────────────────────────────────────┘
```
