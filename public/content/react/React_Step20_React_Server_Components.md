...
```
기존 모델(CSR/SSR 공통)의 구조적 한계

  어떤 방식이든 클라이언트 번들에는 포함된다:
  ┌──────────────────────────────────────────────────┐
  │  클라이언트 JS 번들                               │
  │  · 모든 페이지 컴포넌트 코드                      │
  │  · 데이터 표시만 하는 컴포넌트 (상호작용 없어도!) │
  │  · marked (마크다운 파서, 200KB+)                │
  │  · highlight.js (코드 하이라이팅, 400KB+)        │
  │  · date-fns (날짜 포맷, 72KB+)                  │
  │  → 상호작용이 전혀 없는 콘텐츠도 전부 포함       │
  └──────────────────────────────────────────────────┘

  결과:
    · 번들이 클수록 다운로드 시간 증가
    · 번들이 클수록 JS 파싱/실행 시간 증가 (특히 모바일)
    · Hydration 비용이 전체 컴포넌트 트리에 걸쳐 발생
    · 서버에서 데이터를 조회하려면 API Route 경유 필수
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · Server Component vs Client Component의 차이          │
│  · "use client" / "use server" 지시어                   │
│  · RSC의 렌더링 과정 (직렬화, 전송, 조합)               │
│  · RSC가 번들 크기와 성능에 미치는 영향                  │
│  · Server/Client 경계 설계 전략                         │
│  · RSC와 SSR의 차이 (가장 혼동하기 쉬운 개념)           │
│  · Server Actions ("use server" 함수)                   │
│  · 합성(Composition) 패턴과 RSC                        │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · Next.js App Router 구현 상세 (Step 21)              │
│  · 데이터 캐싱 전략 상세 (Step 23)                      │
│  · 배포 인프라 (Step 40~41)                             │
└─────────────────────────────────────────────────────────┘
```
...
책임 분리 관점의 RSC 모델
  ...
  서버의 책임:              클라이언트의 책임:
  ─────────────────────    ─────────────────────
  · 데이터 조회 (DB, API)   · 사용자 입력 처리
  · 비즈니스 로직 실행      · 상태(State) 관리
  · 민감한 로직 보호        · 애니메이션, 전환 효과
  · 정적 콘텐츠 렌더링      · 브라우저 API 접근
  · 무거운 라이브러리 실행  · 실시간 업데이트
         ↓                         ↓
...
│  JS 번들:   포함되지 않음 ★                                   │
│  할 수 있는 것:                                               │
│    ✅ async/await (비동기 컴포넌트)                           │
│    ✅ 데이터베이스 직접 조회                                   │
│    ✅ 파일 시스템 접근 (fs.readFile 등)                       │
│    ✅ 환경 변수(서버 전용 비밀키) 접근                        │
│    ✅ 무거운 라이브러리 사용 (번들에 안 들어감)               │
│  할 수 없는 것:                                               │
...
│  ───────────────────────────────                             │
│  실행 환경:  브라우저 (+ SSR 시 서버에서도 사전 렌더링)       │
│  JS 번들:   포함됨                                           │
│  할 수 있는 것:                                               │
│    ✅ useState, useEffect 등 모든 Hook                       │
│    ✅ 이벤트 핸들러 (onClick, onChange)                      │
│    ✅ 브라우저 API 접근                                      │
│    ✅ 상호작용이 있는 UI                                     │
│  할 수 없는 것:                                               │
│    ❌ async 컴포넌트 (직접 await 불가)                       │
│    ❌ 서버 전용 자원 접근 (DB, fs, 서버 비밀키)              │
│    ❌ Server Component를 import (역방향 불가 ★)              │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
렌더링 패러다임 관계도
  ...
  CSR: 클라이언트에서 모든 것을 처리
       모든 컴포넌트 코드 → 번들 → 브라우저 실행

  SSR: 서버에서 HTML 생성, 클라이언트에서 Hydration
       서버+클라이언트 양쪽에서 동일한 코드 실행

  RSC: 컴포넌트를 서버 전용 / 클라이언트 전용으로 명시적 분리
       Server Component → 서버에서만, 번들 미포함
       Client Component → 기존 React와 동일

  RSC + SSR (실제 사용 조합):
    Server Component → RSC Payload (직렬화)
                              +
    Client Component → SSR HTML 생성 → 클라이언트 Hydration
    → 두 결과가 결합: 최적의 초기 HTML + 최소 번들 동시 달성


  ┌─────────────────────────────────────────────────────┐
  │  CSR: 브라우저에서 전부                              │
  │  SSR: 서버에서 HTML, 브라우저에서 Hydration(전부)   │
  │  RSC: 서버에서 Server 부분, 브라우저에서 Client만   │
  │       → "서버에서 할 것"과 "클라이언트에서 할 것"의 │
  │          명시적 분리가 핵심                          │
  └─────────────────────────────────────────────────────┘
```
