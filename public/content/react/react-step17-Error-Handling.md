...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ 이벤트 핸들러 에러 (onClick, onChange 등)                │    │
│  │   → try/catch + State                                   │    │
│  │   → showBoundary() (react-error-boundary)               │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ 비동기 에러 (fetch, setTimeout, Promise)                 │    │
│  │   → try/catch + State (기존 방식)                        │    │
│  │   → use() + Error Boundary (React 19)                   │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  계층적 방어선:                                                   │
│  전역 → 라우트 → 기능 → 컴포넌트                                  │
│  (폭발 반경 최소화 = Graceful Degradation)                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│             React 에러의 3가지 영역                           │
│                                                              │
│  1. 렌더링 에러 (Render Phase)                               │
│     · 컴포넌트 함수 실행 중 throw                            │
│     · null.property 접근, 타입 에러 등                       │
│     → Error Boundary가 잡는다 ★                             │
│                                                              │
│  2. 이벤트 핸들러 에러                                       │
│     · onClick, onChange 등 핸들러 안에서 throw               │
│     → Error Boundary가 잡지 못한다!                          │
│     → try/catch로 직접 처리                                  │
│                                                              │
│  3. 비동기 에러                                              │
│     · useEffect 안의 fetch 실패                              │
│     · setTimeout/Promise 안의 에러                           │
│     → Error Boundary가 잡지 못한다!                          │
│     → try/catch + State로 처리                               │
│     → React 19: use() + Suspense + Error Boundary로 처리    │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · Error Boundary의 구조와 동작 원리                     │
│  · Error Boundary가 잡는 에러 / 잡지 못하는 에러        │
│  · 계층적 Error Boundary 설계 전략                       │
│  · 이벤트 핸들러와 비동기 에러 처리 패턴                 │
│  · Error Boundary + Suspense 결합                       │
│  · Fallback UI 설계와 UX 원칙                           │
│  · 에러 복구 전략 (리셋, 재시도)                         │
│  · 전역 에러 핸들링 패턴                                 │
│  · Phase 2 전체 통합 복습                                │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · Sentry 등 모니터링 도구 설정 (Step 42)               │
│  · 서버 사이드 에러 처리 (Step 21)                      │
│  · 테스트에서의 에러 시나리오 검증 (Step 36~37)          │
└─────────────────────────────────────────────────────────┘
```
...
│                에러 처리의 계층 구조                           │
│                                                              │
│  최상위 ─── 전역 Error Boundary                              │
│              · 앱 전체의 최후 방어선                          │
│              · "예상치 못한 오류가 발생했습니다"               │
│              │                                               │
│  페이지 ─── 라우트별 Error Boundary                          │
│              · 특정 페이지의 에러를 격리                      │
│              · 다른 페이지는 정상 동작                        │
│              │                                               │
│  섹션 ───── 기능별 Error Boundary                            │
│              · 특정 위젯/패널의 에러를 격리                   │
│              · 예: 채팅 위젯 에러 → 나머지 대시보드 정상      │
│              │                                               │
│  컴포넌트 ── try/catch + State                               │
│              · 이벤트 핸들러, 비동기 에러                     │
│              · 컴포넌트 레벨의 에러 상태 관리                 │
│                                                              │
└─────────────────────────────────────────────────────────────┘
...
```
┌─────────────────┬──────────────────────────┬────────────────────┐
│  에러 발생 위치  │  Error Boundary가 잡는가  │  처리 도구         │
├─────────────────┼──────────────────────────┼────────────────────┤
│  렌더링 중       │  ✅ 잡는다               │  Error Boundary    │
│  생명주기 메서드 │  ✅ 잡는다               │  Error Boundary    │
│  이벤트 핸들러   │  ❌ 못 잡는다            │  try/catch + State │
│  setTimeout     │  ❌ 못 잡는다            │  try/catch + State │
│  fetch / async  │  ❌ 못 잡는다            │  try/catch + State │
│  SSR            │  ❌ 못 잡는다            │  서버 측 처리      │
│  use() reject   │  ✅ 잡는다 (React 19)    │  Error Boundary    │
└─────────────────┴──────────────────────────┴────────────────────┘
```
