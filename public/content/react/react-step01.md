...
│    │  Node.js (런타임)                                     │     │
│    │    │                                                  │     │
│    │    ├── 패키지 매니저 ──── package.json ──── node_modules │
│    │    │   (npm/pnpm/yarn)    (설계도)         (설치된 패키지)│
│    │    │                                                  │     │
│    │    └── Vite (빌드 도구)                                │     │
│    │         ├── 개발 서버 (Dev) ── ESModule ── HMR         │     │
│    │         └── 빌드 (Prod) ──── Rollup ── 번들/최적화     │     │
│    │                                                       │     │
│    │  VS Code (편집기)                                      │     │
│    │    ├── ESLint (품질)                                   │     │
│    │    └── Prettier (스타일)                               │     │
...
│    브라우저 (런타임)                                              │
│    ┌──────────────────────────────────────────────────────┐     │
│    │  번들된 JS + CSS + 에셋 → React가 DOM 생성·업데이트    │     │
│    └──────────────────────────────────────────────────────┘     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
개발자가 작성한 코드                  브라우저가 실행하는 코드
┌──────────────────┐                ┌──────────────────┐
│  JSX 문법         │    변환 도구    │  순수 JavaScript  │
│  ES Module       │  ──────────→   │  번들된 파일       │
│  TypeScript      │   (Vite 등)    │  최적화된 CSS     │
│  최신 JS 문법     │                │  정적 에셋        │
└──────────────────┘                └──────────────────┘
```
...
┌─────────────────────────────────────────────────┐
│  다루는 것                                       │
│  · Node.js 런타임과 패키지 매니저의 역할          │
│  · 빌드 도구(Vite)의 동작 원리                   │
│  · React 프로젝트 생성과 디렉토리 구조 분석       │
│  · 개발 서버의 동작 메커니즘                      │
│  · 코드 편집기 설정                              │
├─────────────────────────────────────────────────┤
│  다루지 않는 것                                   │
│  · React 문법 (Step 4~)                          │
│  · TypeScript 설정 (Step 31)                     │
│  · 배포 환경 구성 (Step 40)                      │
│  · Docker/CI 설정 (Step 41)                      │
└─────────────────────────────────────────────────┘
```
...
│    │    │                                                    │
│    │    └── Vite (빌드 도구)                                 │
│    │         ├── 개발 서버 (Dev Server + HMR)                │
│    │         └── 프로덕션 빌드 (Rollup 기반 번들링)           │
│    │                                                         │
│    └── VS Code (코드 편집기)                                 │
│         ├── ESLint (코드 품질 검사)                           │
│         ├── Prettier (코드 포매팅)                            │
...
```
┌────────────────┬───────────────────────────────────────────┐
│  개발 타임      │ 개발자가 코드를 작성하고 로컬에서 테스트    │
│  (Development) │ · VS Code에서 코드 편집                    │
│                │ · Vite 개발 서버가 실시간 변환·제공          │
│                │ · HMR로 즉시 결과 확인                      │
├────────────────┼───────────────────────────────────────────┤
│  빌드 타임      │ 소스 코드를 배포 가능한 형태로 변환         │
│  (Build)       │ · JSX → JavaScript 변환 (Transpile)       │
│                │ · 여러 파일 → 소수 파일로 합침 (Bundle)     │
│                │ · 코드 압축·최적화 (Minify)                 │
│                │ · Tree Shaking (미사용 코드 제거)           │
├────────────────┼───────────────────────────────────────────┤
│  런타임         │ 사용자의 브라우저에서 코드가 실행           │
│  (Runtime)     │ · 번들된 JS 파일 다운로드                  │
│                │ · React가 DOM을 생성·업데이트               │
│                │ · 사용자와 상호작용                         │
└────────────────┴───────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────┐
│  Node.js의 역할 (개발·빌드 시점에만 사용)                 │
│                                                          │
│  1. 패키지 관리  → npm/pnpm이 라이브러리 설치·관리        │
│  2. 개발 서버    → Vite가 파일을 변환하고 브라우저에 제공   │
│  3. 빌드 도구    → Vite/Rollup이 번들링·최적화 수행       │
│  4. 린팅/포매팅  → ESLint, Prettier가 코드 품질 검사      │
│  5. 테스트 실행  → Vitest 등 테스트 러너가 Node.js에서 실행│
│                                                          │
│  ※ 최종 사용자의 브라우저에서는 Node.js가 실행되지 않는다  │
└─────────────────────────────────────────────────────────┘
```
...
버전 선택 기준
  ...
  LTS (짝수 번호: 18.x, 20.x, 22.x)
  ├── 30개월간 보안 패치 지원
  ├── 프로덕션 환경에 권장
  └── 안정성 우선

  Current (홀수 번호: 19.x, 21.x, 23.x)
  ├── 최신 기능 포함
  ├── 6개월간만 지원
  └── 실험·학습용
```
...
nvm의 동작 원리
  ...
  nvm use 20  →  현재 셸의 PATH를 v20.18.0으로 전환
  nvm use 22  →  현재 셸의 PATH를 v22.11.0으로 전환
```
...
· 평탄화(hoisting) 전략 사용
· 같은 패키지가 프로젝트마다 중복 저장됨
· Lock 파일: package-lock.json


pnpm
───────────────────────────────
~/.pnpm-store/              ← 글로벌 공유 저장소 (Content-Addressable)
  └── react@19.0.0/

node_modules/
  ├── .pnpm/                ← 하드 링크로 글로벌 저장소 참조
  │    └── react@19.0.0/
  └── react → .pnpm/react@19.0.0/node_modules/react  (심볼릭 링크)

· 패키지를 글로벌 저장소에 한 번만 저장하고 하드 링크로 참조
· 디스크 절약, 설치 속도 향상
· Phantom Dependency 방지 (선언하지 않은 패키지 접근 차단)
...
───────────────────────────────
.yarn/cache/
  ├── react-npm-19.0.0-xxxxx.zip
  └── vite-npm-6.0.0-xxxxx.zip

· Plug'n'Play (PnP) 모드: node_modules 폴더 없이 동작
· .pnp.cjs 파일이 패키지 위치를 직접 매핑
· Zero-Install: 캐시를 git에 커밋하여 설치 없이 실행 가능
· Lock 파일: yarn.lock
```
...
```
상황: 프로젝트가 A 패키지만 설치했는데, A가 B를 의존한다

npm (평탄화)
───────────────
node_modules/
  ├── A/
  └── B/     ← A의 의존성이지만 평탄화로 최상위에 노출

· 코드에서 import B 가 동작함 (package.json에 B를 선언하지 않았는데!)
· A가 업데이트되어 B를 더 이상 사용하지 않으면?
  → B가 사라져서 코드가 깨짐 (Phantom Dependency 버그)
...
node_modules/
  ├── .pnpm/
  │    ├── A@1.0.0/
  │    │    └── node_modules/B/  ← B는 A 내부에만 존재
  │    └── B@2.0.0/
  └── A → .pnpm/A@1.0.0/

· import B → 실패! (최상위에 B가 없음)
· package.json에 명시적으로 추가해야만 사용 가능
· 의존성 관계가 정직하게 유지됨
```
...
개발 서버 시작 시:
  ...
  모든 파일 스캔 → 의존성 그래프 구축 → 전체 번들 생성 → 서버 시작
  ──────────────────────────────────────────────────────────────
  시간: 수 초 ~ 수십 초 (프로젝트 규모에 비례)

파일 변경 시:

  변경 감지 → 관련 모듈 재번들 → HMR 전송
  ────────────────────────────────────────
  시간: 수백 ms ~ 수 초
...
개발 서버 시작 시:
  ...
  서버 즉시 시작 → 브라우저가 요청한 파일만 변환하여 제공
  ──────────────────────────────────────────────────────
  시간: 수백 ms (프로젝트 규모와 거의 무관)

파일 변경 시:

  변경 감지 → 해당 모듈 1개만 변환 → HMR 전송
  ──────────────────────────────────────────
  시간: 수십 ms (거의 즉시)
```
...
│                   Vite의 이중 구조                    │
│                                                      │
│  1. 개발 모드 (vite / vite dev)                      │
│     ┌─────────────────────────────────────────┐     │
│     │  · 네이티브 ESModule을 그대로 활용        │     │
│     │  · 번들링 없이 개별 파일을 변환·제공       │     │
│     │  · esbuild로 JSX/TS를 초고속 변환         │     │
│     │  · HMR으로 변경분만 교체                  │     │
│     └─────────────────────────────────────────┘     │
│                                                      │
...
│                      │
│  2. 브라우저가 main.jsx 요청
│     └→ Vite가 JSX → JS 변환 후 제공
│        (esbuild 사용, ~1ms)
│                      │
│  3. main.jsx 안의 import 문 발견
│     └→ import App from './App.jsx'
│     └→ 브라우저가 App.jsx 요청
│     └→ Vite가 App.jsx 변환 후 제공
│                      │
│  4. 연쇄적으로 필요한 파일만 변환·제공
│     └→ 사용하지 않는 파일은 변환하지 않음!
└──────────────────────┘
```
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. Node.js는 개발 도구의 실행 환경이다                        │
│     → 빌드 도구, 패키지 매니저, 린터가 Node.js 위에서 동작     │
│     → 최종 사용자의 브라우저에서는 실행되지 않는다              │
│     → LTS 버전 사용을 권장한다                                │
│                                                               │
│  2. 패키지 매니저는 의존성의 단일 진실 공급원을 관리한다        │
│     → npm: 기본 제공, 범용                                    │
│     → pnpm: 디스크 효율, Phantom Dependency 방지              │
│     → package.json + lock 파일 = 재현 가능한 환경              │
│                                                               │
│  3. Vite는 ESModule 기반의 차세대 빌드 도구이다                │
│     → 개발 서버: on-demand 변환 (번들링 없음)                  │
│     → 프로덕션: Rollup 기반 최적화 번들링                      │
│     → CRA를 대체하는 현재의 표준                               │
│                                                               │
│  4. 개발 타임 / 빌드 타임 / 런타임을 구분한다                  │
│     → 각 시점에서 동작하는 도구와 코드가 다르다                 │
│     → dependencies vs devDependencies의 근거                  │
│                                                               │
│  5. 프로젝트 구조에서 각 파일의 역할을 안다                    │
│     → index.html: Vite의 진입점                               │
│     → src/main.jsx: React의 진입점                            │
│     → public/ vs src/assets/: 정적 에셋 처리 방식의 차이       │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
