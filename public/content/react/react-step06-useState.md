...
│                                                               │
│  주의점:                                                      │
│    · setState(count + 1) × 3 = 1 증가 (스냅샷 값 사용)       │
│    · setState(p => p + 1) × 3 = 3 증가 (이전 큐 결과 사용)   │
│    · setTimeout 내 State = 호출 시점의 스냅샷 (Stale Closure) │
│                                                               │
│  bailout: Object.is(prev, next) === true면 렌더링 생략        │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
│  다루는 것                                               │
│  · useState의 동작 원리                                  │
│  · State는 스냅샷이다 (Snapshot 모델)                    │
│  · Batching과 State 업데이트 큐                         │
│  · Updater Function (함수형 업데이트)                    │
│  · Stale Closure 문제와 해결                             │
│  · StrictMode의 이중 실행                                │
│  · State 초기화 전략 (Lazy Initialization)               │
│  · State 설계 원칙과 안티 패턴                           │
│  · 객체/배열 State의 Immutable 업데이트                  │
│  · bailout 최적화와 Object.is                           │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · useReducer (Step 13)                                 │
│  · useEffect (Step 11)                                  │
│  · useMemo / useCallback (Step 14)                      │
│  · Context에 의한 State 공유 (Step 25)                  │
└─────────────────────────────────────────────────────────┘
```
...
사용자 액션 (클릭, 입력 등)
       ...
       ▼
┌──────────────────┐
│  이벤트 핸들러    │  setState 호출들이 큐에 쌓인다
│  실행             │  Batching: 핸들러가 끝날 때까지 대기
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  State 큐 처리    │  쌓인 업데이트를 순서대로 처리
│                   │  교체(value) or 함수(updater) 구분
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  재렌더링         │  컴포넌트 함수 호출 (새 스냅샷 생성)
│  (Render Phase)  │  새 React Element 트리 생성
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  Reconciliation  │  이전 트리 vs 새 트리 비교 (Step 7)
│  (Diff)          │  변경된 부분 파악
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  DOM 업데이트     │  변경분만 실제 DOM에 반영
│  (Commit Phase)  │  브라우저가 화면을 다시 그림
└──────────────────┘
```
...
```
Automatic Batching 적용 범위

  ┌────────────────────┬───────────┬───────────┐
  │  상황               │ React 17  │ React 18+ │
  ├────────────────────┼───────────┼───────────┤
  │  이벤트 핸들러      │  ✅ 배칭   │  ✅ 배칭  │
  │  setTimeout        │  ❌ 개별   │  ✅ 배칭  │
  │  Promise.then      │  ❌ 개별   │  ✅ 배칭  │
  │  fetch 콜백        │  ❌ 개별   │  ✅ 배칭  │
  │  addEventListener  │  ❌ 개별   │  ✅ 배칭  │
  │  queueMicrotask    │  ❌ 개별   │  ✅ 배칭  │
  └────────────────────┴───────────┴───────────┘
```
...
│  상황             │  해결책                 │  추천도           │
├──────────────────┼────────────────────────┼──────────────────┤
│  setState에서      │  Updater Function      │  ⭐⭐⭐           │
│  이전 값 필요      │  prev => prev + 1      │  가장 일반적       │
├──────────────────┼────────────────────────┼──────────────────┤
│  타이머/이벤트에서 │  useRef로 최신 값       │  ⭐⭐⭐           │
│  최신 값 읽기      │  동기화                │  읽기 전용일 때    │
├──────────────────┼────────────────────────┼──────────────────┤
│  useEffect 내부에서│  의존성 배열에          │  ⭐⭐             │
│  값 참조          │  해당 값 추가           │  Step 11에서 학습  │
└──────────────────┴────────────────────────┴──────────────────┘
```
...
```
"이 값을 State로 만들어야 하는가?"

  ┌─ 시간이 지나도 변하지 않는가?
  │    YES → State 아님 (상수로 선언)
  │
  ├─ Props로 전달받는가?
  │    YES → State 아님 (Props 그대로 사용)
  │
  ├─ 기존 State/Props로 계산 가능한가?
  │    YES → State 아님 (렌더링 중 계산 = Derived State)
  │
  └─ 위 모두 아닌가?
       YES → State로 만든다 ✅
```
...
├──────────────────┼───────────────────────────────────┤
│  .map()          │  .push()      → [...arr, new]     │
│  .filter()       │  .pop()       → .slice(0, -1)     │
│  .slice()        │  .shift()     → .slice(1)         │
│  .concat()       │  .unshift()   → [new, ...arr]     │
│  [...spread]     │  .splice()    → .slice + 조합      │
│  .toSorted() ★   │  .sort()      → [...arr].sort()   │
│  .toReversed() ★ │  .reverse()   → [...arr].reverse() │
│  .toSpliced() ★  │  arr[i] = x   → .map()으로 교체   │
└──────────────────┴───────────────────────────────────┘
★ ES2023 — 원본을 변경하지 않는 새 버전
```
