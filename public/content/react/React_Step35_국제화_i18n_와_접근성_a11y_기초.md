...
```
   ...
   ┌──────┼──────┐    ┌─────┼─────┐    ┌─────┼──────┐
   │      │      │    │     │     │    │     │      │
 번역키  보간  Intl  lang  RTL  aria  WCAG 시맨틱 키보드
 관리  동적값  API  속성 레이아웃 label 4원칙  HTML  접근성
          │                 │                 │
          └─────────────────┼─────────────────┘
                            │
                 ┌──────────┼──────────┐
                 │          │          │
           ┌─────▼─────┐ ┌──▼──┐ ┌────▼────┐
           │react-i18next│ │Intl │ │axe/     │
           │LanguageDetector│ API│ │Lighthouse│
           └───────────┘ └─────┘ └─────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · i18n 아키텍처: react-i18next 기반                    │
│  · 번역 키 관리, 네임스페이스, 동적 언어 전환            │
│  · 날짜/숫자/통화 포맷팅 (Intl API)                     │
│  · WCAG 4대 원칙과 핵심 기준                             │
│  · 시맨틱 HTML: 올바른 요소 선택                        │
│  · ARIA: 역할, 상태, 속성                                │
│  · 키보드 접근성: 포커스 관리, Tab 순서                  │
│  · 색상 대비, 텍스트 크기, 모션 감소                     │
│  · 접근성 진단 도구 (axe, Lighthouse)                   │
│  · Phase 5 전체 통합 복습                                │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · RTL 레이아웃 구현 상세                                │
...
│                                                           │
│  ┌─────────────────────────────────────────────────────┐ │
│  │              i18n 설정 (i18n/config.ts)              │ │
│  │  · LanguageDetector → 브라우저 언어 자동 감지        │ │
│  │  · resources → 번역 파일 등록                        │ │
│  │  · fallbackLng → 번역 없을 때 기본 언어              │ │
│  └────────────────────────┬────────────────────────────┘ │
│                           │                               │
│         ┌─────────────────┼─────────────────┐             │
│         │                 │                 │             │
...
│  │  t('common.save') → "저장" (현재 언어 기준)        │  │
│  │  i18n.changeLanguage('en') → 전체 앱 언어 전환     │  │
│  └──────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
```
