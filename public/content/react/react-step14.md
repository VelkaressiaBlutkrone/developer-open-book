...
│  · React.memo: Props 비교로 컴포넌트 재렌더링 건너뛰기   │
│  · 세 도구의 관계와 결합 패턴                            │
│  · "언제 쓰지 말아야 하는가" — 과도한 최적화의 비용      │
│  · React Profiler를 활용한 측정 기반 판단                │
│  · React Compiler가 메모이제이션에 미치는 영향           │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · 가상화(Virtualization) — windowing 기법               │
│  · Web Worker 활용                                      │
│  · 서버 사이드 최적화 (Step 19~21)                      │
│  · 번들 최적화, Tree Shaking (Step 39)                  │
└─────────────────────────────────────────────────────────┘
```
...
```
JavaScript에서 참조 동일성(Reference Equality)

  // 원시값: 값으로 비교
  42 === 42           → true  (같은 값)
  'hello' === 'hello' → true  (같은 값)

  // 객체/배열/함수: 참조로 비교
  {} === {}           → false (다른 참조, 다른 메모리 주소)
  [] === []           → false (다른 참조)
  (() => {}) === (() => {})  → false (다른 참조)

  const a = {}
  const b = a
  a === b             → true  (같은 참조, 같은 메모리 주소)
```

```
React 컴포넌트에서 매 렌더링마다 일어나는 일
  ...
  function Parent() {
    ...
    return <Child config={config} onClick={handleClick} ids={ids} />;
  }

결과:
  · Child가 React.memo로 감싸져 있어도
  · 모든 Props가 매 렌더링마다 "다른 참조"이므로
  · React.memo의 얕은 비교: "Props가 변했다!" → 재렌더링
  · React.memo가 완전히 무용지물이 된다
```
...
│               메모이제이션 도구의 관계                        │
│                                                              │
│  문제: 부모가 재렌더링될 때 자식도 재렌더링된다              │
│                                                              │
│  ┌──────────────────────────────────────────────────┐       │
│  │  Parent (재렌더링됨)                              │       │
│  │                                                   │       │
│  │  const data = expensiveCalc(items);  ← 매번 재계산│       │
...
│         ▼                                                    │
│  ┌──────────────────────────────────────────────────┐       │
│  │  Child (React.memo로 감싸져 있어도 재렌더링!)      │       │
│  │  Props가 변했다고 판단됨 (참조가 다르므로)         │       │
│  └──────────────────────────────────────────────────┘       │
│                                                              │
│  해결:                                                       │
│  ① useMemo(expensiveCalc, [items]) → data 참조 안정화       │
│  ② useCallback(handler, [deps])    → handler 참조 안정화    │
│  ③ React.memo(Child)               → Props 비교로 건너뛰기  │
│  → ①②로 참조를 안정화하고 ③으로 비교하면 재렌더링 방지       │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
