...
TypeScript = JavaScript + 정적 타입 시스템
  ...
  · 코드를 실행하기 전에 타입 에러를 발견한다 (컴파일 타임)
  · IDE 자동완성이 정확해진다 (Props 목록, 메서드 제안)
  · 리팩토링이 안전해진다 (타입이 변하면 사용처를 자동 추적)
  · 문서 역할을 한다 (타입 정의 = 인터페이스 문서)
  · 실무 React 프로젝트의 90%+ 가 TypeScript를 사용

TypeScript가 React에서 특히 빛나는 이유:
  · Props의 타입을 정의하면 → 잘못된 Props 전달을 컴파일에서 차단
  · Event Handler의 타입 → e.target.value가 어떤 타입인지 자동 추론
  · Hook의 제네릭 → useState<User>(null)로 정확한 타입 추론
  · Context의 타입 → Provider/Consumer의 타입 일치 보장
```

### 1.4 TypeScript와 React의 관계 개념 지도

```
TypeScript + React 개념 지도

  ┌──────────────────────────────────────────────────────────┐
  │              TypeScript가 React를 강화하는 영역           │
  │                                                          │
  │   컴포넌트 레이어          타입 도구                     │
  │   ─────────────           ─────────────                 │
  │   Props 정의       →      interface / type               │
  │   children 타입    →      ReactNode / ReactElement       │
  │   HTML 속성 확장   →      ComponentProps<'button'>       │
  │   조건부 Props     →      Discriminated Union            │
  │                                                          │
  │   Hook 레이어              타입 도구                     │
  │   ─────────────           ─────────────                 │
  │   useState         →      제네릭 <T>                     │
  │   useReducer       →      판별 유니온 Action             │
  │   useContext       →      null 안전 Custom Hook          │
  │   useRef           →      HTML 요소 타입 지정            │
  │                                                          │
  │   재사용 레이어            타입 도구                     │
  │   ─────────────           ─────────────                 │
  │   Generic Component →     <T> 타입 매개변수              │
  │   API 타입 설계    →      Partial, Pick, Omit, Record    │
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · 컴포넌트 Props 타입 정의 (interface vs type)         │
│  · children, Event, Ref의 타입                          │
│  · Hook의 타입 (useState, useReducer, useContext, useRef)│
│  · Generic Component (재사용 가능한 타입)                │
│  · 유틸리티 타입 (Partial, Pick, Omit, Record 등)       │
│  · ComponentProps를 활용한 HTML 속성 확장                │
│  · 타입 설계 원칙과 실전 패턴                            │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · TypeScript 기초 문법 (별도 학습 가정)                │
│  · tsconfig.json 세부 설정                              │
│  · 타입 가드, 조건부 타입 등 고급 타입 프로그래밍       │
│  · Zod 등 런타임 검증 (Step 32)                         │
└─────────────────────────────────────────────────────────┘
```
...
ReactElement (좁은 타입):
  · <Component /> 또는 <div />처럼 JSX로 만들어진 것만
  · "children은 반드시 JSX여야 한다"는 제약이 필요할 때
  → Tabs, Carousel처럼 children을 직접 조작하는 컴포넌트에 사용
```
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. TypeScript = 실행 전에 버그를 잡는 안전망                  │
│     → Props 타입 정의 → 잘못된 Props 전달을 컴파일에서 차단  │
│     → IDE 자동완성 → 생산성 향상                             │
│     → 타입 = 문서 → 코드만으로 인터페이스 파악               │
│                                                               │
│  2. Props 타입 패턴                                           │
│     → interface로 정의, 선택적 Props에 ? 사용                │
│     → children: ReactNode (가장 넓은 타입)                   │
│     → Discriminated Union: 조건부 Props 안전하게 정의        │
│     → ComponentProps<'button'>: HTML 속성 확장               │
│                                                               │
│  3. Event 타입: ChangeEvent, FormEvent, MouseEvent 등         │
│     → 제네릭: ChangeEvent<HTMLInputElement>                  │
│     → 인라인 핸들러에서는 자동 추론에 맡기기                  │
│                                                               │
│  4. Hook 타입                                                 │
│     → useState<T>: 초기값에서 추론, 복잡한 타입은 명시        │
│     → useReducer: 판별 유니온으로 Action 타입 정의            │
│     → useContext + null 체크: Custom Hook에서 처리            │
│     → useRef<HTMLElement>(null): DOM 타입 지정               │
│                                                               │
│  5. Generic Component로 타입 안전한 재사용 컴포넌트 구현       │
│     → List<T>, Select<T>, DataTable<T>                       │
│     → 사용 시 T가 자동 추론됨                                │
│                                                               │
│  6. 유틸리티 타입으로 반복을 줄인다                            │
│     → Partial(수정), Pick(선택), Omit(제외), Record(매핑)    │
│     → API 타입: 생성=Omit, 수정=Partial, 목록=Pick           │
│                                                               │
│  7. 타입 설계 원칙                                            │
│     → 좁은 타입을 사용한다 (string보다 리터럴 유니온)         │
│     → any를 피하고 unknown을 사용한다                        │
│     → 타입을 도메인 가까이 정의한다 (Co-location)            │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
