...
```
   ...
   └──────┬──────┘      └──────┬──────┘      └──────┬──────┘
    ...
    ┌─────┼─────┐         ┌────┼────┐          ┌─────┼─────┐
    │     │     │         │    │    │          │     │     │
 onSubmit onBlur onChange 메시지 시각  위치    ARIA  키보드 포커스
                         4원칙  설계              속성  탐색  관리
          │                     │                     │
          └─────────────────────┼─────────────────────┘
                                │
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · 검증 시점 전략 (인라인 / 제출 시 / 실시간)            │
│  · 에러 메시지의 시각적·의미적 설계                      │
│  · 폼 접근성(a11y): ARIA 속성, 키보드 네비게이션         │
│  · 접근성을 갖춘 FormField 컴포넌트 설계                │
│  · 자동 저장(Auto-save) 패턴                            │
...
검증 시점: 빨리 ←──────────────────────────→ 늦게
  ...
  실시간 (onChange)    인라인 (onBlur)    제출 시 (onSubmit)
  ─────────────        ──────────────     ──────────────────
  · 매 키 입력마다      · 필드를 떠날 때   · Submit 클릭 시
  · 즉각적 피드백       · 적절한 피드백    · 한 번에 모든 에러
  · 입력 중 방해 가능   · 입력 방해 없음   · 제출 후에야 인지
  · 성능 비용 높음      · 균형잡힌 비용    · 성능 비용 낮음
  · 이메일 중복 체크    · 대부분의 폼에    · 간단한 폼에 적합
    등 특수 경우에 적합   가장 권장 ★
```
...
│                                                           │
│  피드백 계층:                                             │
│    시각적 피드백 (눈) ──→ 프로그래밍적 피드백 (스크린 리더)│
│    ┌──────────────┐       ┌──────────────────┐           │
│    │ · 빨간 테두리│       │ · aria-invalid   │           │
│    │ · 에러 텍스트│       │ · aria-describedby│          │
│    │ · 에러 아이콘│       │ · role="alert"   │           │
│    └──────────────┘       └──────────────────┘           │
│                                                           │
│  상태 관리:                                               │
│    RHF ──→ formState ──→ UI 업데이트                     │
│    · mode/reValidateMode  · errors        · 에러 표시    │
│    · isDirty              · isSubmitting  · 저장 표시    │
│    · dirtyFields          · isValid       · 진행률      │
└──────────────────────────────────────────────────────────┘
```
...
│       ↓                                                     │
│  <input                                                     │
│    id="email-input"          ← label과 연결                 │
│    aria-invalid="true"       ← 에러 상태 선언               │
│    aria-describedby="err help" ← 설명 요소들과 연결         │
│    aria-required="true"      ← 필수 항목 선언               │
│  />                                                         │
│       │                                                     │
│       │ [aria-describedby 연결]                             │
│       ↓                                                     │
│  <p id="err" role="alert">   ← 에러 메시지 (즉시 읽힘)     │
│    유효하지 않은 이메일                                      │
│  </p>                                                       │
│  <p id="help">               ← 도움말 텍스트               │
│    업무용 이메일을 입력하세요                                │
...
```
  ┌─────────────────┬──────────────┬──────────────┬────────────────┐
  │                 │  onSubmit    │  onBlur      │  Submit→onChange│
  ├─────────────────┼──────────────┼──────────────┼────────────────┤
  │  첫 에러 인지   │  제출 후     │  필드 떠날 때│  제출 후        │
  │  에러 수정 피드백│  다시 제출   │  다시 blur   │  즉시 (onChange)│
  │  입력 중 방해   │  없음        │  없음        │  에러 후에만    │
  │  사용자 좌절감  │  높음        │  낮음        │  가장 낮음 ★    │
  │  구현 복잡도    │  낮음        │  중간        │  중간           │
  │  RHF mode      │  'onSubmit'  │  'onBlur'    │  mode+reValidate│
  └─────────────────┴──────────────┴──────────────┴────────────────┘
```
...
│  ARIA 속성           │  역할                                    │
├─────────────────────┼──────────────────────────────────────────┤
│  aria-invalid="true"│  필드가 에러 상태임을 스크린 리더에 알림 │
│  aria-describedby   │  에러 메시지/도움말의 ID를 연결          │
│  aria-required      │  필수 입력임을 알림 (HTML required 보완)  │
│  aria-errormessage  │  에러 메시지 요소의 ID (aria-invalid과 결합)│
│  role="alert"       │  에러 메시지가 나타나면 즉시 읽어줌 (live region) │
│  aria-live="polite" │  변경 사항을 적절한 시점에 읽어줌        │
│  aria-hidden="true" │  장식용 요소를 스크린 리더에서 숨김 (아이콘 등) │
└─────────────────────┴──────────────────────────────────────────┘

접근성의 최소 요구사항:
  1. 모든 input에 연결된 <label> (htmlFor + id)
  2. 에러 시 aria-invalid="true"
  3. 에러 메시지를 aria-describedby로 연결
  4. 에러 메시지에 role="alert" (즉시 읽힘)
  5. 필수 필드에 aria-required="true" 또는 required
  6. 키보드만으로 모든 필드 탐색 및 제출 가능
```
