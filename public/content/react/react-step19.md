...
```
HTML 생성 위치 × 시점 — 전략별 위치

  ┌──────────────────┬──────────────────┬──────────────────────────┐
  │                  │  빌드 시점        │  요청 시점                │
  ├──────────────────┼──────────────────┼──────────────────────────┤
  │  서버에서 생성    │  SSG             │  SSR / Streaming SSR     │
  │                  │  (Static)        │  (Server)                │
  ├──────────────────┼──────────────────┼──────────────────────────┤
  │  클라이언트에서  │  —               │  CSR                     │
  │  생성            │                  │  (Client)                │
  ...
  ISR = SSG + 주기적 재생성 (빌드 시점 + 요청 시점의 하이브리드)
  Streaming SSR = SSR + 점진적 전송 (서버에서 생성하되 조각으로 보냄)
...
전략 간 관계도
  ...
  과거 (전통적 SSR)
       │
       ▼
  클라이언트 중심 (CSR) ──→ SEO·초기로딩 문제 발견
       │
       ▼
  서버 복귀 (현대 SSR) ──→ Hydration 비용 문제 발견
       │             │
       │             └──→ SSG (정적 = 빠름, but 빌드시 고정)
       │                    │
       │                    └──→ ISR (SSG + 주기 갱신)
       │
       └──→ Streaming SSR + Selective Hydration (현재 최전선)


성능 지표 관점 우선순위

  최고 TTFB :  SSG > Streaming > CSR > SSR
  최고 FCP  :  SSG > SSR > Streaming > CSR
  최고 SEO  :  SSG = SSR > ISR > CSR
  최저 비용  :  SSG/CSR(CDN) > ISR > SSR
```
...
│  · 혼합 전략(Hybrid Rendering)                          │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · React Server Components 상세 (Step 20)              │
│  · Next.js 구현 상세 (Step 21)                          │
│  · 서버 구축/배포 (Step 40~41)                          │
└─────────────────────────────────────────────────────────┘
```
...
```
사용자가 URL을 입력한 후의 시간 흐름

  요청 ──→ TTFB ──→ FCP ──→ LCP ──→ TTI
  │         │        │       │       │
  │         │        │       │       └─ 모든 상호작용 가능 (클릭, 입력)
  │         │        │       └─────── 가장 큰 콘텐츠 표시 (히어로 이미지 등)
  │         │        └─────────────── 첫 번째 콘텐츠 표시 (텍스트, 이미지)
  │         └──────────────────────── 서버 응답 첫 바이트 도착
  └────────────────────────────────── 브라우저가 서버에 요청

  각 렌더링 전략은 이 지표들에 서로 다른 영향을 준다
...
성능 지표와 렌더링 전략의 관계
  ...
  지표       영향 요소                    유리한 전략
  ──────     ──────────────────────────  ──────────────────
  TTFB       서버 처리 시간, CDN 여부     SSG (CDN 즉시 반환)
  FCP        HTML 콘텐츠 포함 여부        SSG = SSR > Streaming > CSR
  LCP        이미지/최대 요소 로딩        SSG = SSR > Streaming > CSR
  TTI        JS 번들 + Hydration 비용     Streaming (Selective Hydration)
  CLS        레이아웃 이동 (레이아웃 안정) SSR/SSG (스켈레톤 정확도)
```
...
타임라인:
  요청 → TTFB(빠름, 정적 HTML) → FCP(느림! JS 실행 후)
  → API 패칭 → LCP(더 느림) → TTI(JS 실행 완료 후)
```
