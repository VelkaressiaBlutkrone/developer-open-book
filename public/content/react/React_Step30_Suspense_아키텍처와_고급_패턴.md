...
Suspense의 진짜 역할
  ...
  표면적: "로딩 중일 때 fallback을 보여주는 컴포넌트"
  본질적: "비동기 작업의 완료를 선언적으로 기다리는 메커니즘" ★

  Suspense가 기다릴 수 있는 것:
    · React.lazy → 코드 청크 로딩 (Step 29)
    · use(Promise) → 데이터 패칭 (React 19)
    · Server Component 렌더링 → Streaming SSR (Step 19)
    · 향후: 이미지 로딩, 스크립트 로딩 등 확장 예정

  핵심: Suspense는 "무엇을 기다리는가"에 관심이 없다.
        "기다리는 동안 무엇을 보여줄 것인가"만 선언한다.
        → 관심사 분리의 극치
```
...
Suspense 아키텍처 개념 지도
  ...
  │   데이터 패칭        →     use(Promise) + Suspense          │
  │   서버 렌더링        →     Streaming SSR + Suspense         │
  │                                │                           │
  │                                ▼                           │
  │              ┌─────────────────────────────┐              │
  │              │      Suspense 경계           │              │
  ...
  │                표시됨         표시됨                        │
  │                                                             │
  │   에러 처리: ErrorBoundary (Suspense 바깥 또는 안쪽)         │
  │   패턴 통합: AsyncBoundary = Suspense + ErrorBoundary       │
  │                                                             │
  └─────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · Suspense의 내부 동작 메커니즘 (throw Promise)        │
│  · 선언적 데이터 패칭: use() + Suspense + ErrorBoundary │
│  · Suspense 경계 배치 전략과 UX 설계                    │
│  · Streaming SSR에서 Suspense의 역할                    │
│  · Skeleton UI와 점진적 로딩 패턴                       │
│  · 중첩 Suspense와 SuspenseList (미래)                  │
│  · 명령적 → 선언적 패턴 전환의 의미                     │
│  · Phase 4 전체 통합 복습                                │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · TanStack Query의 Suspense 모드 상세                  │
│  · Next.js의 loading.js 상세 (Step 21에서 학습)        │
│  · Offscreen/Activity API (실험적)                      │
└─────────────────────────────────────────────────────────┘
```
...
명령적 (Step 22에서 학습한 방식):
  ...
  function UserProfile({ userId }) {
    ...
    if (isLoading) return <Spinner />;        ← 로딩 처리가
    if (error) return <ErrorMsg />;            ← 컴포넌트 안에
    return <Profile user={user} />;            ← 섞여 있다
  }

  · 컴포넌트가 "데이터 패칭 + 로딩 처리 + UI 렌더링" 모두 담당
  · 3가지 관심사가 하나의 컴포넌트에 혼합
  · 로딩 UI를 바꾸려면 컴포넌트 내부를 수정해야 함
...
선언적 (Suspense 방식):
  ...
  function UserProfile({ userId }) {
    const user = use(fetchUser(userId));  ← 데이터 사용만!
    return <Profile user={user} />;       ← UI 렌더링만!
  }

  · 로딩 → Suspense가 처리 (밖에서 선언)
  · 에러 → ErrorBoundary가 처리 (밖에서 선언)
  · 컴포넌트 → 데이터가 있는 상태에서 UI만 렌더링
  · 3가지 관심사가 각각 분리됨 ★
  · 로딩 UI를 바꾸려면 Suspense fallback만 수정
```
...
Suspense의 내부 (개념적 설명)
  ...
  1. 컴포넌트가 렌더링된다
  2. 컴포넌트 내부에서 use(promise)를 호출한다
  3. Promise가 아직 pending이면:
     · React가 Promise를 "throw"한다 (에러가 아닌 Promise를 던짐!)
     · 가장 가까운 <Suspense>가 이를 잡는다 (ErrorBoundary와 유사)
     · <Suspense>가 fallback을 렌더링한다
  4. Promise가 resolve되면:
     · React가 컴포넌트를 다시 렌더링한다
     · use(promise)가 resolve된 값을 반환한다
     · fallback이 실제 컴포넌트로 교체된다
  5. Promise가 reject되면:
  ...
  function Suspense({ fallback, children }) {
    try {
      ...
      if (thrown instanceof Promise) {
        show(fallback);
        thrown.then(() => retry(children));
      } else {
        throw thrown; // 일반 에러 → ErrorBoundary가 처리
      }
    }
  }

  ⚠️ 실제 내부 구현은 Fiber Architecture와 결합되어 훨씬 복잡하지만
     "Promise를 throw하고 Suspense가 잡는다"라는 멘탈 모델은 정확하다
```

#### React.lazy의 내부 (Suspense 관점)

```
React.lazy가 Suspense와 결합하는 원리
  ...
  동작:
  1. <LazyComponent /> 렌더링 시도
  2. import('./Heavy')가 아직 로드되지 않음 → Promise 상태
  3. lazy 내부에서 해당 Promise를 throw
  4. <Suspense>가 잡아서 <Spinner /> 표시
  5. JS 청크 다운로드 완료 → Promise resolve
  6. <LazyComponent />를 다시 렌더링 → 실제 컴포넌트 표시

  use(promise)도 동일한 원리로 동작:
  1. use(fetchData())가 Promise를 만남
  2. pending이면 → throw (Suspense가 잡음)
  3. resolve되면 → 값 반환 (컴포넌트 렌더링 계속)
  4. reject되면 → throw error (ErrorBoundary가 잡음)
```
...
이 패턴의 핵심 가치
  ...
  1. 컴포넌트는 "데이터가 있다"고 가정하고 작성한다
     · if (isLoading) return ... → 필요 없음!
     · if (error) return ... → 필요 없음!
     · "Happy Path"만 작성

  2. 로딩 처리는 Suspense가 선언적으로 담당한다
     · <Suspense fallback={<Skeleton />}>
     · "이 영역이 로딩 중이면 Skeleton을 보여줘"

  3. 에러 처리는 ErrorBoundary가 선언적으로 담당한다
     · <ErrorBoundary fallback={<Error />}>
     · "이 영역에서 에러가 나면 Error를 보여줘"

  4. 세 가지 관심사가 완전히 분리된다
     · 데이터 사용 → 컴포넌트
     · 로딩 처리 → Suspense
     · 에러 처리 → ErrorBoundary
```
...
```
같은 페이지, 다른 Suspense 경계 → 완전히 다른 사용자 경험


전략 A: 페이지 전체를 하나의 Suspense로

  <Suspense fallback={<FullPageSkeleton />}>
    <Header />          ← 모든 것이 하나의 로딩에 묶임
    <UserInfo />        ← 하나라도 로딩 중이면
    <PostList />        ← 전체가 스켈레톤
    <Sidebar />
  </Suspense>

  · UserInfo가 50ms, PostList가 500ms면?
  · UserInfo가 빨리 준비되어도 PostList를 기다려야 함
  · 500ms 동안 전체 페이지가 스켈레톤
  · 사용자: "너무 느리다"


전략 B: 각 섹션을 독립 Suspense로
  ...
  · Header와 Sidebar는 즉시 표시 (Suspense 밖)
  · UserInfo: 50ms 후 스켈레톤 → 실제 UI
  · PostList: 500ms 후 스켈레톤 → 실제 UI
  · 각 섹션이 독립적으로 로딩 → 더 빠른 체감 속도!
  · 사용자: "빠르게 채워지고 있다"


전략 C: 중요도에 따른 계층적 Suspense

  <Suspense fallback={<PageShell />}>         ← 가장 바깥: 페이지 전체
    <Header />
    <Suspense fallback={<UserInfoSkeleton />}> ← 핵심 콘텐츠
      <UserInfo />
    </Suspense>
    <Suspense fallback={<PostListSkeleton />}> ← 부가 콘텐츠
      <PostList />
    </Suspense>
    <Suspense fallback={<SidebarSkeleton />}>  ← 부가 콘텐츠
      <Sidebar />
    </Suspense>
  </Suspense>

  · 가장 바깥 Suspense: 전체 데이터가 없을 때 기본 셸
  · 안쪽 Suspense들: 각 섹션 독립 로딩
  · 네트워크가 느려도 셸은 즉시 표시
```
...
```
Suspense 경계를 어디에 둘 것인가?

  1. "이 영역이 독립적으로 로딩되어야 하는가?"
     YES → 별도 Suspense 경계 생성
     NO → 부모 Suspense에 포함

  2. "로딩 중에 이 영역 대신 무엇을 보여줄 것인가?"
     → 이것이 Skeleton UI 설계의 기준

  3. "이 데이터의 로딩 시간이 다른 것과 크게 다른가?"
     YES → 분리 (느린 것이 빠른 것을 차단하지 않도록)
     NO → 합칠 수 있음

  4. "부분적으로 표시하는 것이 의미가 있는가?"
     프로필 + 게시글: 프로필만 먼저 보여주는 것이 의미 있음 → 분리
     결제 폼: 부분만 보이면 혼란 → 하나로 묶기

  원칙: "사용자가 가장 빨리, 가장 많은 콘텐츠를 볼 수 있도록" 배치
```
...
```
Step 19에서 배운 Streaming SSR의 실체가 바로 Suspense이다

  서버에서:
  1. React가 컴포넌트 트리를 렌더링한다
  2. <Suspense> 경계를 만나면:
     · 자식이 suspend하면 → fallback의 HTML을 먼저 전송
     · 자식이 완료되면 → 실제 HTML을 "청크"로 추가 전송
  3. 각 Suspense 경계가 독립적으로 스트리밍된다

  클라이언트에서:
  4. fallback HTML이 먼저 표시된다 (스켈레톤)
  5. 서버에서 청크가 도착하면 fallback을 실제 콘텐츠로 교체
  6. 해당 영역만 Selective Hydration (Step 19)

  Suspense 경계 = Streaming의 청크 단위 = Hydration의 단위

  <Suspense fallback={<A_Skeleton />}>
    <ComponentA />                    ← 청크 1: A가 준비되면 전송
  </Suspense>
  <Suspense fallback={<B_Skeleton />}>
    <ComponentB />                    ← 청크 2: B가 준비되면 전송 (A와 독립)
  </Suspense>

  → A가 200ms, B가 500ms면
  → A의 HTML이 200ms에 도착, B의 스켈레톤이 계속 표시
  → B의 HTML이 500ms에 도착, 스켈레톤이 교체
  → "점진적으로 채워지는" 페이지 경험!
```
...
useSuspenseQuery vs useQuery
  ...
  useQuery:
    · isLoading, isError, data를 컴포넌트에서 직접 처리
    · data가 undefined일 수 있음 → 조건부 렌더링 필요
    · 명령적 패턴

  useSuspenseQuery:
    · 로딩 → Suspense가 처리, 에러 → ErrorBoundary가 처리
    · data가 항상 정의됨 → 조건부 렌더링 불필요
    · 선언적 패턴
    · TanStack Query의 캐싱 + Suspense의 선언적 UX 결합

  선택 기준:
    · Suspense 경계를 이미 설계한 앱 → useSuspenseQuery
    · 기존 앱에 점진적 적용 → useQuery (호환성)
    · 세밀한 로딩 제어 필요 → useQuery
```
...
React의 패러다임 전환 역사
  ...
  렌더링:
    명령적: el.textContent = content;
    선언적: return <div>{content}</div>;   ← React가 DOM을 관리

  상태 업데이트:
    명령적: element.textContent = newValue;
    선언적: setState(newValue);            ← React가 UI를 동기화

  라우팅:
    명령적: window.location = '/page';
    선언적: <Route path="/page" element={<Page />} />;

  에러 처리:
    명령적: if (error) return <ErrorUI />;
    선언적: <ErrorBoundary fallback={<ErrorUI />}>;

  로딩 처리 (가장 최근의 전환):
    명령적: if (isLoading) return <Spinner />;    ← Step 22
    선언적: <Suspense fallback={<Spinner />}>;    ← 이 Step ★

  이 모든 전환의 공통점:
    · "어떻게(How)"를 React에 위임한다
    · 개발자는 "무엇을(What)" 선언한다
    · 관심사가 분리된다
    · 코드가 간결해진다

  Suspense는 이 여정의 마지막 퍼즐 조각이다:
    "데이터 로딩"이라는 가장 흔한 비동기 작업을 선언적으로 처리
```

### 3.9 Phase 4 전체 통합 복습

```
Phase 4 (Step 25~30)에서 배운 것

  Step 25: Context API
           · Props Drilling 해결, Provider + useContext
           · 리렌더링 문제와 최적화 4전략
           · 적합한 데이터: 변경 드문 전역 값 (테마, 인증)

  Step 26: Zustand
           · selector 기반 부분 구독 → Context 한계 해결
           · Provider 불필요, React 트리 밖 접근 가능
           · 미들웨어 (persist, devtools, immer)
           · 상태 관리 도구 최종 선택 가이드

  Step 27: 컴포넌트 설계 패턴
           · Compound Component (구조의 유연성)
           · Headless Component (최대 커스터마이즈)
           · 유연성의 단계: Props < Slot < Compound < Headless
           · 패턴의 진화: HOC → Render Props → Hook

  Step 28: 프로젝트 구조
           · Feature-based vs Layer-based
           · Screaming Architecture, Co-location
           · 의존성 방향: pages → features → shared
           · Barrel Export로 모듈 경계 정의

  Step 29: 성능 최적화 심화
           · Core Web Vitals (LCP, INP, CLS)
           · 코드 분할 (React.lazy + Suspense)
           · 번들 분석, 이미지 최적화, Virtualization
           · Lighthouse 진단 + 최적화 우선순위

  Step 30: Suspense 아키텍처 (이 Step)
           · Suspense의 진짜 역할 (선언적 비동기 처리)
           · use() + Suspense + ErrorBoundary = 선언적 데이터 패칭
           · 경계 배치가 UX를 결정한다
           · Streaming SSR의 청크 단위 = Suspense 경계


  Phase 4의 핵심 메시지:
    "상태를 목적에 맞는 도구로 관리하고 (Context/Zustand/TanStack Query)
     컴포넌트를 재사용 가능하게 설계하며 (Compound/Headless)
     코드를 체계적으로 조직하고 (Feature-based)
     성능을 측정 기반으로 최적화하며 (Lighthouse/Profiler)
     로딩과 에러를 선언적으로 처리한다 (Suspense/ErrorBoundary)"

  Phase 5부터는 이 기반 위에 타입 안전성, 폼, 스타일링을 완성한다.
```
...
❌ 설계 A: 하나의 Suspense로 전체 감싸기
  ...
  → 300ms(가장 느린 C)까지 전체 스켈레톤 표시
  → 사용자 체감: 느림


✅ 설계 B: 위젯별 독립 Suspense
  <AsyncBoundary pendingFallback={<ChartSkeleton />} ...>
    <RevenueChart />          → 200ms 후 표시
  </AsyncBoundary>
  <AsyncBoundary pendingFallback={<StatsSkeleton />} ...>
    <VisitorStats />          → 100ms 후 표시 (가장 먼저!)
  </AsyncBoundary>
  <AsyncBoundary pendingFallback={<OrdersSkeleton />} ...>
    <RecentOrders />          → 300ms 후 표시 (가장 나중)
  </AsyncBoundary>
  <AsyncBoundary pendingFallback={<NotiSkeleton />} ...>
    <Notifications />         → 150ms 후 표시
  </AsyncBoundary>

  → 각 위젯이 준비되는 대로 독립적으로 표시
  → 100ms에 방문자 통계, 150ms에 알림, 200ms에 차트, 300ms에 주문
  → 사용자 체감: "빠르게 채워지고 있다!"
  → 한 위젯 에러 → 나머지 위젯 정상 동작 (Graceful Degradation)
```
...
```
Next.js App Router에서 Suspense는 파일 규약으로 자동 적용됨

  app/dashboard/
  ├── layout.js      → 레이아웃 (즉시 표시)
  ├── page.js        → 메인 콘텐츠 (Server Component, async)
  ├── loading.js     → = <Suspense fallback={<Loading />}> 자동 적용!
  └── error.js       → = <ErrorBoundary> 자동 적용!

  파일만 만들면 Suspense 아키텍처가 완성된다:
    · loading.js → Suspense fallback (스켈레톤)
    · error.js → ErrorBoundary (에러 UI)
    · page.js → async Server Component (데이터 패칭)
    · 개발자는 각 파일만 작성하면 프레임워크가 조합

  Step 21에서 배운 "특수 파일 규약"의 진짜 의미:
    Next.js가 Suspense + ErrorBoundary 아키텍처를
    파일 시스템 규약으로 추상화한 것이다!
```
...
```
요구사항:
  · 대시보드에 3개 위젯: 사용자 정보(100ms), 통계(300ms), 최근 활동(500ms)
  · 인위적 지연: await new Promise(r => setTimeout(r, delay))
  · 3가지 설계로 구현:
    A: 전체를 하나의 Suspense로
    B: 위젯별 독립 Suspense
    C: 계층적 Suspense (바깥=페이지 셸, 안쪽=위젯별)
  · 각 설계에서 "첫 콘텐츠가 보이는 시간"과 "전체 완료 시간" 기록

기록표:
  | 설계 | 첫 콘텐츠 표시 | 전체 완료 | 중간 상태 |
  |-----|--------------|----------|----------|
  |  A  | ?ms          | ?ms      | 전체 스켈레톤 |
  |  B  | ?ms          | ?ms      | 개별 스켈레톤 |
  |  C  | ?ms          | ?ms      | 셸+개별 스켈레톤 |
...
```
과제:
  · Step 22의 수동 패칭 코드(UserProfile)를 Suspense 패턴으로 변환
  · AsyncBoundary 재사용 컴포넌트 구현
  · useSuspenseQuery(TanStack Query)로도 같은 기능 구현
  · 두 방식의 코드량과 복잡도 비교

비교할 것:
  | 항목        | 명령적 (Step 22) | use() + Suspense | useSuspenseQuery |
  |------------|-----------------|------------------|-----------------|
  | State 변수  | ?개             | ?개              | ?개             |
  | useEffect  | ?개             | ?개              | ?개             |
  | 조건부 렌더링| ?개             | ?개              | ?개             |
...
요구사항:
  · 상품 카드의 Skeleton 구현
    ...
    - 제목 (텍스트 줄)
    - 가격 (짧은 텍스트 줄)
    - 평점 (별 아이콘 placeholder)
    - shimmer 애니메이션
  · 상품 카드 Skeleton과 실제 상품 카드의 레이아웃이 동일한지 검증
  · CLS 측정: Skeleton → 실제 교체 시 레이아웃 이동 = 0

  추가:
  · 페이지 레벨 Skeleton (전체 레이아웃)
  · 리스트 Skeleton (카드 그리드 반복)
```
...
```
과제 (개념적 분석):
  아래 컴포넌트 구조에서 Streaming SSR이 어떻게 동작하는지 분석하라.

  <Layout>
    <Header />                          ← 즉시 HTML
    <Suspense fallback={<HeroSkeleton />}>
      <HeroSection />                   ← 데이터 패칭 200ms
    </Suspense>
    <Suspense fallback={<ProductsSkeleton />}>
      <ProductGrid />                   ← 데이터 패칭 400ms
    </Suspense>
    <Suspense fallback={<ReviewsSkeleton />}>
      <ReviewSection />                 ← 데이터 패칭 600ms
    </Suspense>
    <Footer />                          ← 즉시 HTML
  </Layout>

  작성할 것:
  1. 서버에서 전송되는 HTML 청크의 순서와 시점
  2. 각 시점에서 사용자가 보는 화면의 상태
  3. Hydration이 어떤 순서로 진행되는지
  4. 사용자가 HeroSection을 클릭하면 어떤 시점부터 반응하는지
```
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. Suspense = "비동기 작업을 선언적으로 기다리는 메커니즘"    │
│     → React.lazy(코드 분할), use()(데이터), Streaming SSR     │
│     → "무엇을 기다리는가"에 관심 없이 fallback만 선언         │
│     → 내부: Promise를 throw하고 Suspense가 catch              │
│                                                               │
│  2. 선언적 데이터 패칭 = use() + Suspense + ErrorBoundary     │
│     → 컴포넌트: 데이터가 있다고 가정하고 작성 ("Happy Path")  │
│     → 로딩: Suspense fallback이 처리                         │
│     → 에러: ErrorBoundary가 처리                             │
│     → 3가지 관심사가 완전히 분리됨                            │
│                                                               │
│  3. Suspense 경계의 위치가 UX를 결정한다 ★                    │
│     → 전체 하나: 가장 느린 데이터가 모든 것을 차단            │
│     → 개별 분리: 각 섹션이 독립적으로 로딩 → 빠른 체감       │
│     → 기준: "독립 로딩이 의미 있는가?", "로딩 시간 차이가 큰가?"│
│                                                               │
│  4. Streaming SSR의 청크 단위 = Suspense 경계                 │
│     → 서버가 Suspense fallback을 먼저 전송                   │
│     → 데이터 준비되면 실제 HTML을 청크로 추가 전송            │
│     → Selective Hydration: 준비된 영역부터 상호작용 활성화    │
│                                                               │
│  5. Skeleton UI가 스피너보다 나은 UX를 제공한다               │
│     → 실제 레이아웃과 일치 → CLS 방지                        │
│     → "곧 콘텐츠가 나타날 것"이라는 기대감 전달              │
│     → shimmer 애니메이션으로 로딩 중 신호                    │
│                                                               │
│  6. AsyncBoundary 패턴으로 Suspense+ErrorBoundary를 통합      │
│     → 재사용 가능한 래퍼 컴포넌트                            │
│     → 위젯별 독립 로딩/에러 → Graceful Degradation           │
│                                                               │
│  7. 명령적 → 선언적 전환은 React의 일관된 철학                │
│     → DOM → React Element (렌더링)                           │
│     → if(isLoading) → <Suspense fallback> (로딩)             │
│     → try/catch → <ErrorBoundary> (에러)                     │
│     → 개발자는 "무엇을" 선언, React가 "어떻게"를 처리        │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
