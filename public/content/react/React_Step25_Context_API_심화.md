...
Context의 핵심 아이디어
  ...
  Context:
    ...
    · Layout과 Sidebar는 theme을 모른다
    · MenuItem이 직접 Context에서 읽는다
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · Context의 생성, 제공, 소비 3단계                      │
│  · React 19의 Context 변경사항                           │
│  · useReducer + Context 결합 패턴                       │
│  · Context 리렌더링 문제와 최적화 전략                   │
│  · Context 분리 전략 (State/Dispatch 분리)              │
│  · Custom Hook으로 Context 캡슐화                       │
│  · Context가 적합한 경우 vs 부적합한 경우               │
│  · Zustand 등 전역 상태 라이브러리와의 비교 (개요)      │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · Zustand 상세 (Step 26)                               │
│  · Redux Toolkit 상세 (Step 26)                         │
│  · 서버 상태와 Context의 관계 (Step 23~24에서 학습)     │
└─────────────────────────────────────────────────────────┘
```
...
Props Drilling과 비교
  ...
  Props Drilling:
    App(theme) → Main(theme) → Sidebar(theme)
    · Main이 theme을 전달만 하기 위해 Props에 포함

  Context:
    App: <ThemeContext.Provider value={theme}>
    Sidebar: const theme = useContext(ThemeContext)
    · Main은 theme의 존재를 모른다! ★
    · Sidebar가 Context에서 직접 접근
```
...
이 패턴의 구조
  ...
  TodoProvider
    ├── TodoContext.Provider (todos 값 제공)
    │     └── TodoDispatchContext.Provider (dispatch 함수 제공)
    │           └── children
    │
    └── 어떤 하위 컴포넌트든:
          · useTodos() → todos 배열 접근
          · useTodoDispatch() → dispatch 함수 접근
          · Props Drilling 없이 직접 접근!

  Custom Hook으로 캡슐화하면:
    · Context의 존재를 컴포넌트가 몰라도 된다
    · Provider 밖에서 사용하면 에러 메시지 표시 (개발자 경험 향상)
    · Context 구현을 변경해도 Hook 인터페이스는 유지
```
...
```
이것이 3.2절에서 TodoContext와 TodoDispatchContext를 분리한 이유이다

  dispatch 함수는 useReducer에 의해 안정적인 참조를 가진다
  (리렌더링되어도 같은 함수 참조)

  TodoContext.Provider value={todos}
    → todos가 변하면 소비자 리렌더링 (예상대로)

  TodoDispatchContext.Provider value={dispatch}
    → dispatch는 참조가 변하지 않으므로 소비자 리렌더링 없음!

  AddTodoForm: useTodoDispatch()만 사용
    → todos가 변해도 리렌더링 안 됨 ★ (dispatch 참조는 동일)

  TodoStats: useTodos()만 사용
    → dispatch가 변해도 리렌더링 안 됨 (어차피 안 변하지만)
```
...
```
children 패턴이 최적화되는 이유

  ThemeProvider가 리렌더링될 때:
    · <ThemeContext.Provider value={...}> → 새 value (리렌더링 트리거)
    · {children} → 이전 렌더링과 동일한 참조! (App에서 생성)
    · React: "children이 변하지 않았으므로 자식 트리 리렌더링 건너뜀"

  만약 children이 아니라 직접 자식을 작성했다면:
    <ThemeContext.Provider value={...}>
      <Header />  ← ThemeProvider가 리렌더링될 때마다 새 Element 생성!
      <Main />    ← 리렌더링!
    </ThemeContext.Provider>

  핵심: children Props로 전달하면 Provider의 리렌더링이
        자식 트리의 불필요한 리렌더링으로 전파되지 않는다
```
...
```
✅ Context가 적합한 데이터 (자주 변하지 않는 전역 값)

  · 테마 (light/dark) — 변경 빈도: 매우 낮음
  · 인증 정보 (현재 사용자) — 변경 빈도: 로그인/로그아웃 시
  · 로케일/언어 설정 — 변경 빈도: 거의 없음
  · 기능 플래그 (feature flags) — 변경 빈도: 배포 시
  · 라우터 정보 (React Router가 내부적으로 Context 사용)

  공통점:
    · 변경 빈도가 낮다
    · 앱 전체에서 필요하다
    · 변경 시 전체 리렌더링이 수용 가능하다


❌ Context가 부적합한 데이터 (자주 변하는 상태)

  · 폼 입력값 — 키 입력마다 변경
  · 마우스 위치 — 지속적으로 변경
  · 애니메이션 값 — 프레임마다 변경
  · 자주 업데이트되는 목록 데이터
  · 여러 컴포넌트가 각각 다른 부분만 필요한 복잡한 상태

  이유: Context value 변경 → 모든 소비자 리렌더링
        자주 변하면 성능 문제 발생!


❌ Context가 부적합한 상황 (Server State)

  · 서버 데이터 (API 응답) — TanStack Query (Step 23)
  · 캐싱, 리패칭, 에러 재시도가 필요한 데이터
  · "Server State를 Context에 넣으려 하지 마라"
    → TanStack Query가 자체적으로 전역 캐시를 관리한다
```
...
선택 흐름도
  ...
  ┌─ 서버에서 오는 데이터인가?
  │    YES → TanStack Query (Step 23). Context 사용하지 않음
  │    NO ↓
  │
  ├─ 자주 변하는 데이터인가? (초당 여러 번)
  │    YES → Zustand 등 전역 상태 라이브러리 (Step 26)
  │    NO ↓
  │
  ├─ 앱 전체에서 필요하고 변경 빈도가 낮은가?
  │    YES → Context API ★
  │
  └─ 특정 컴포넌트 트리에서만 필요한가?
...
```
┌──────────────────┬───────────────────────┬───────────────────────┐
│                  │  Context API          │  Zustand (Step 26)    │
├──────────────────┼───────────────────────┼───────────────────────┤
│  설치            │  없음 (React 내장)     │  npm install zustand │
│  보일러플레이트   │  많음 (Context +       │  적음 (create 함수)   │
│                  │  Provider + Hook)     │                       │
│  리렌더링 최적화 │  수동 (분리, useMemo)  │  자동 (selector 기반) │
│  DevTools        │  없음                 │  있음                 │
│  학습 곡선       │  낮음                 │  낮음                 │
│  Provider 필요   │  필수                 │  불필요               │
│  적합한 규모     │  소규모, 변경 드문 값  │  중~대규모, 빈번한 변경│
│  Server State    │  부적합               │  부적합 (TanStack Query)│
└──────────────────┴───────────────────────┴───────────────────────┘

요약:
  · Context: 테마, 인증, 로케일 등 "앱 설정" 수준의 값에 적합
  · Zustand: 자주 변하거나 부분 구독이 필요한 복잡한 상태에 적합
  · TanStack Query: 서버 데이터는 항상 이것으로 (Context/Zustand 아님)
```
...
```
구조 비교: Props Drilling vs Context + useReducer

  Props Drilling:
    App
    ├── [todos, dispatch를 모든 자식에 전달]
    ├── AddForm(dispatch)
    ├── FilterBar(filter, setFilter)
    ├── TodoList(filteredTodos, dispatch)
    │   └── TodoItem(todo, dispatch)
    └── Stats(todos)

  Context + useReducer:
    TodoProvider(useReducer → Context 제공)
    └── App
        ├── AddForm    → useTodoDispatch()
        ├── FilterBar  → 로컬 State만
        ├── TodoList   → useTodos() + useTodoDispatch()
        │   └── TodoItem → useTodoDispatch()
        └── Stats      → useTodos()

  변경점:
    · Props에서 todos/dispatch가 사라짐
    · 각 컴포넌트가 필요한 Context만 직접 접근
    · 새 컴포넌트 추가 시 Props 체인 수정 불필요
```
...
시나리오: 실시간 주식 대시보드
  ...
  10개 주식 종목이 각각 초당 1회 업데이트된다
  → 전체 State가 초당 10회 변경

  Context로 구현하면:
    StockContext.Provider value={stocks}
    → stocks가 초당 10회 변경
    → 가격만 보는 컴포넌트, 차트만 보는 컴포넌트 등
      모든 소비자가 초당 10회 리렌더링!
    → 성능 재앙

  Zustand로 구현하면:
    const useStockStore = create((set) => ({ stocks: [] }))
    const applePrice = useStockStore(state => state.stocks.find(s => s.id === 'AAPL')?.price)
    → AAPL 가격이 변할 때만 이 컴포넌트가 리렌더링
    → 다른 종목 변경에 무반응
    → selector 기반 부분 구독으로 성능 보장

  결론: 자주 변하는 + 부분 구독이 필요한 상태 → Zustand (Step 26)
```
...
```
실험:
  Step 1: 하나의 Context에 모든 값을 넣는다
    AppContext: { user, theme, counter, sidebarOpen }
    → 4개 소비자 컴포넌트를 만들고 각각 console.log
    → counter를 빠르게 증가시키며 다른 컴포넌트의 리렌더링 관찰

  Step 2: Context를 분리한다
    UserContext, ThemeContext, CounterContext, UIContext
    → 같은 실험 반복 → counter만 리렌더링되는지 확인

  Step 3: useMemo로 value 안정화
    → useMemo 적용 전후의 리렌더링 횟수 비교

  기록:
    | 최적화 단계 | counter 변경 시 리렌더링되는 컴포넌트 수 |
    |------------|----------------------------------------|
    | Step 1     | ?개                                    |
    | Step 2     | ?개                                    |
    | Step 3     | ?개                                    |
...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  1. Context = Props 없이 트리를 관통하여 데이터를 전달한다     │
│     → createContext → Provider(값 제공) → useContext(값 소비) │
│     → 중간 컴포넌트가 Props를 전달하지 않아도 된다            │
│     → React 19: <Context value={...}> (.Provider 생략)       │
│                                                               │
│  2. useReducer + Context = 전역 상태 관리 패턴                │
│     → Provider에서 useReducer로 State 생성                   │
│     → Context로 State와 dispatch를 하위 트리에 제공           │
│     → Custom Hook으로 캡슐화 (useCart, useAuth 등)           │
│                                                               │
│  3. Context의 리렌더링 문제를 이해해야 한다 ★                 │
│     → Provider value 변경 → 모든 소비자 리렌더링             │
│     → React.memo로도 방지 불가 (useContext는 memo 우회)       │
│     → 자주 변하는 값을 Context에 넣으면 성능 문제             │
│                                                               │
│  4. 리렌더링 최적화 4가지 전략                                │
│     → Context 분리: 관심사별로 다른 Context 사용              │
│     → State/Dispatch 분리: dispatch는 안정적 참조            │
│     → useMemo로 value 안정화: 객체 참조 안정화               │
│     → children 패턴: Provider의 리렌더링이 자식에 전파 방지   │
│                                                               │
│  5. Context가 적합한 데이터: 변경이 드문 전역 값              │
│     → 테마, 인증, 로케일, 기능 플래그                        │
│     → 변경 빈도가 낮고, 앱 전체에서 필요한 값                │
│                                                               │
│  6. Context가 부적합한 데이터                                 │
│     → 자주 변하는 상태 (폼 입력, 카운터, 위치) → Zustand     │
│     → 서버 데이터 (API 응답) → TanStack Query               │
│     → 부분 구독이 필요한 복잡한 상태 → Zustand               │
│                                                               │
│  7. Custom Hook으로 Context를 캡슐화한다                      │
│     → 컴포넌트가 Context의 존재를 모르게 한다                │
│     → Provider 밖에서 사용 시 명확한 에러 메시지              │
│     → 구현 변경 시 Hook 인터페이스 유지                       │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
