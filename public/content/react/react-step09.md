...
│                                                               │
│  리스트 렌더링                                                │
│    ├── array.map(item => <Component key={id} />)             │
│    ├── key: 고유 식별자 (Step 7 복습)                         │
│    └── 파생 리스트: filter → sort → map (원본 불변)           │
│                                                               │
│  4가지 UI 상태                                                │
│    ├── 로딩 중 → 스피너/스켈레톤                              │
│    ├── 에러 발생 → 에러 메시지 + 재시도                       │
│    ├── 빈 상태 → Empty State + 행동 유도                      │
│    └── 데이터 표시 → 리스트/콘텐츠                            │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────┐
│  다루는 것                                               │
│  · 조건부 렌더링 6가지 패턴과 선택 기준                   │
│  · && 연산자의 함정과 방지법                              │
│  · React가 렌더링하지 않는 값 / 렌더링하는 값             │
│  · 리스트 렌더링의 map 패턴과 key 복습                   │
│  · 빈 배열, 로딩, 에러 상태의 UI 설계                    │
│  · 리스트 필터링, 정렬, 그룹화 패턴                      │
│  · 중첩 리스트와 트리 구조 렌더링                        │
├─────────────────────────────────────────────────────────┤
│  다루지 않는 것                                          │
│  · 가상화(Virtualization) — 대규모 리스트 최적화          │
│  · Suspense 기반 로딩 UI (Step 30)                      │
│  · Error Boundary (Step 17)                             │
│  · 서버 데이터 패칭 패턴 (Step 22~23)                   │
└─────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────┐
│  React가 렌더링하지 않는 값 (화면에 아무것도 표시 안 됨)  │
│                                                          │
│    false                                                 │
│    null                                                  │
│    undefined                                             │
│    true                                                  │
│                                                          │
│  → 조건부 렌더링에서 "아무것도 표시하지 않음"에 활용       │
│                                                          │
├─────────────────────────────────────────────────────────┤
│  React가 렌더링하는 값 (화면에 표시됨!)  ⚠️ 주의         │
│                                                          │
│    0         → 문자 "0"이 화면에 표시됨                  │
│    NaN       → 문자 "NaN"이 화면에 표시됨                │
│    ''        → 빈 문자열 (표시되지만 보이지 않음)         │
│    'hello'   → "hello" 표시                              │
│    42        → "42" 표시                                 │
│    <p>Hi</p> → React Element로 렌더링                    │
│    [1, 2]    → "12" 표시 (배열 요소가 나열됨)            │
│                                                          │
└─────────────────────────────────────────────────────────┘
```
...
│  상황                          │  권장 패턴                  │
├────────────────────────────────┼────────────────────────────┤
│  로딩/에러/빈 상태 분기         │  Guard Clause (조기 반환)   │
│  A 또는 B (인라인, 단순)       │  삼항 연산자                │
│  보이거나 / 안 보이거나         │  && 연산자                 │
│  데이터 없을 때 기본값          │  || 또는 ??               │
│  3개 이상 분기                 │  변수 할당 (switch) 또는    │
│                                │  객체 매핑                  │
│  설정 기반 동적 렌더링          │  객체 매핑                  │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                               │
│  1. 조건부 렌더링 6가지 패턴                                  │
│     → Guard Clause: 로딩/에러/빈 상태를 조기 반환             │
│     → 삼항 연산자: JSX 인라인에서 A 또는 B 선택               │
│     → && 연산자: 보이거나 / 안 보이거나                       │
│     → || / ??: 기본값(Fallback) 제공                         │
│     → 변수 할당: 복잡한 분기 (switch)                        │
│     → 객체 매핑: 4개 이상 분기의 깔끔한 처리                  │
│                                                               │
│  2. && 연산자의 숫자 0 함정                                   │
│     → 0 && <Component /> → 화면에 "0" 표시!                  │
│     → 해결: count > 0 && ... 또는 !!count && ...             │
│     → 규칙: && 왼쪽에 숫자가 올 수 있으면 명시적 비교 필수    │
│                                                               │
│  3. React가 렌더링하지 않는 값: false, null, undefined, true  │
│     React가 렌더링하는 값: 0, NaN, 문자열, Element            │
│                                                               │
│  4. 리스트 렌더링 = array.map() + key                        │
│     → map의 최상위 Element에 고유한 key 부여                  │
│     → key는 데이터의 ID 사용 (index는 조건부 허용)            │
│                                                               │
│  5. 필터/정렬/검색은 파생 데이터로 처리한다                    │
│     → 원본 State를 변경하지 않는다                            │
│     → filter().sort().map() 체이닝                           │
│     → .sort()는 반드시 [...arr].sort()로 복사 후 사용         │
│                                                               │
│  6. 데이터 표시 UI의 4가지 상태를 항상 설계한다                │
│     → Loading → Error → Empty → Data 순서로 Guard Clause     │
│     → Empty State에는 행동 유도 메시지 포함                   │
│                                                               │
│  7. CSS 숨기기 vs 조건부 렌더링의 트레이드오프                 │
│     → CSS: State 보존, DOM 유지, 빠른 토글                   │
│     → 조건부 렌더링: State 파괴, DOM 제거, 메모리 절약         │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
